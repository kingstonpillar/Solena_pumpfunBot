...
import { TOKEN_PROGRAM_ID, Token } from '@solana/spl-token';  // Corrected import
...
async function sellAllTokens(mintAddressStr) {
  try {
    ...
    const tokenAccountAddress = await getOrCreateTokenAccount(wallet, mintAddress);  // Retrieve or create token account
    ...
    // Fetch receive token dynamically from the .env (can be either USDC or SOL)
    const receiveToken = process.env.RECEIVE_TOKEN;
    if (!receiveToken) {
      throw new Error('❌ RECEIVE_TOKEN missing in .env');
    }

    const receiveTokenInstance = orca.getToken(receiveToken);
    const solToken = orca.getToken('SOL');  // SOL as the receiving token

    const pool = await orca.getPool(receiveTokenInstance, solToken);  // Use dynamic receive token
