...
import pkg from '@orca-so/sdk';  // Default import for CommonJS module
const { Orca, Network } = pkg;  // Destructure Orca and Network

// Initialize Orca instance
const orca = new Orca(Network.MAINNET);  

// Check if Orca instance is initialized
if (!orca) {
  console.error('Orca instance not initialized!');
  process.exit(1);  // Stop the execution if Orca initialization failed
} else {
