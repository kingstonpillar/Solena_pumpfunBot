...
import { Orca, Network } from '@orca-so/sdk';  // Destructure Orca and Network from the updated SDK

// Initialize Orca properly
const orca = new Orca(Network.MAINNET);  // Use this if getInstance() is not available

// Check if the Orca instance is initialized correctly
if (!orca) {
  console.error('Orca instance not initialized!');
  process.exit(1);  // Exit the process if Orca initialization fails
} else {
  console.log('Orca instance initialized successfully');
}
