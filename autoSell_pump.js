...
import { AbortController } from 'abort-controller';  // Ensure compatibility for Node.js environments
import {
  Connection,
  PublicKey,
  Keypair,
...
async function fetchJupJson(url, body = null) {
  const controller = new AbortController(); // Create AbortController to manage timeout
  const timeout = setTimeout(() => controller.abort(), JUP_TIMEOUT_MS); // Set timeout

  try {
    const headers = {
      "Content-Type": "application/json",
      "User-Agent": "SolenaPumpfunBot/1.0",
    };

    if (JUP_API_KEY) {
      headers["x-api-key"] = JUP_API_KEY;  // Include API key if available
    }

    const response = await fetch(url, {
      method: body ? "POST" : "GET", // Use POST if body is provided
      headers: headers,
      body: body ? JSON.stringify(body) : null,
      signal: controller.signal,  // Use the AbortController signal for timeouts
    });
    ...
    return json; // Return parsed JSON if the response is successful
  } catch (error) {
    // Log and handle errors, including timeout and HTTP errors
