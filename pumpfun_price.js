...
export async function getPumpFunPriceOnce(record, connection) {
  const conn = connection || makeConn();

  if (!record?.mint) return { error: "missing_mint" };

  // Resolve FIRST (handles ...pump, token account, bonding curve, etc)
const resolved = await resolveMintOnChain(conn, record?.mint, "confirmed");
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const out = await getPumpFunPriceOnce({ mint }); // no need to pass connection now
