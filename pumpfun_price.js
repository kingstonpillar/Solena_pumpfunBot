...
export async function getPumpFunPriceOnce(record, connection) {
  if (!record?.mint) return { error: "missing_mint" };
  if (!connection) return { error: "missing_connection" };

  // 0) Resolve ANY pump / non-32 / token-account / curve → real mint
  const resolved = await resolveMintOnChain(connection, record);
  ...
  const mint = String(resolved.mint).trim();

  // Resolve decimals safely (prefer record.decimals only if valid)
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const out = await getPumpFunPriceOnce({ mint }, connection);
