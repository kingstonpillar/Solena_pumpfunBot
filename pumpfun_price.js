...
async function getBondingCurvePrice(record) {
  let curve = record?.bondingCurve;

  // NEW: if not provided, derive PDA from mint
  if (!curve && record?.mint) {
    try {
      curve = deriveBondingCurvePda(record.mint);
    } catch {
      return { error: "invalid_mint_for_curve_pda" };
    }
  }
  ...
  const { virtualTokenReserves, virtualSolReserves } = parseBondingCurveAccount(acc.data);
