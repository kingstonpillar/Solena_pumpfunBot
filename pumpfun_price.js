...
// 3) DexScreener (fallback if Jup has no coverage)
//
// Notes:
...
import { getPriceUsdFromDexscreener } from "./dexscreener_price.js";
...
// Optional: queue for DexScreener too (keeps your limiter consistent)
...
async function fetchJson(
  url,
  { label, timeoutMs = 12000, headers = {}, retries = 3 } = {}
) {
  let lastErr = null;

  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const res = await withTimeout(fetch(url, { headers }), timeoutMs, label || "fetch");
...
async function getDexScreenerPriceSOL(mint) {
  const now = Date.now();
  const cached = dexCache.get(mint);
  if (cached && now - cached.ts <= DEX_CACHE_TTL_MS) return cached.data;

  // Get token USD from DexScreener
  const { priceUsd, pair } = await dexQ.add(() =>
    getPriceUsdFromDexscreener(mint)
  );

  // Need SOL USD to convert USD -> SOL
  const SOL_MINT = "So11111111111111111111111111111111111111112";
  const sol = await getJupiterPriceSOL(SOL_MINT);

  const solUsd = num(sol?.tokenUsd || sol?.solUsd); // handle either structure
  if (!solUsd || solUsd <= 0) {
    const out = {
...
export async function getPumpFunPriceOnce(record) {
  ...
  if (dex?.priceSOL && dex.priceSOL > 0) return dex;
  ...
  return {
    ...
    dexscreenerError: dex?.error || null,
  };
}

// Optional: keep a direct USD helper if you still want it elsewhere.
export async function getPumpfunPriceUsd(mint) {
  const { priceUsd, pair } = await dexQ.add(() => getPriceUsdFromDexscreener(mint));
  return {
    mint,
    priceUsd,
    source: "dexscreener",
