...
const inputSOL = Number(process.env.BUY_INPUT_SOL || 50);
const inputSol = inputSOL;
...
function rpcLimited(opName, fn) {
  // rate-limit + failover using SIGNER_URL_1 and SIGNER_URL_2
  return withRpcFailover(opName, (c) => fn(c));
}
...
// ---------------- Jupiter Helper----------------
// Initialize the jupQueue for rate-limiting Jupiter API calls
...
async function findTokenAccountAndProgram(connection, ownerPubkey, mintPubkey) {
  const spl = await connection.getParsedTokenAccountsByOwner(ownerPubkey, {
  ...
  for (const a of spl.value) {
    ...
    if (info.mint === mintPubkey.toBase58()) {
      return { tokenAccount: a.pubkey, programId: TOKEN_PROGRAM_ID };
    }
  }

  const t22 = await connection.getParsedTokenAccountsByOwner(ownerPubkey, {
  ...
  for (const a of t22.value) {
    const info = a.account.data.parsed.info;
    if (info.mint === mintPubkey.toBase58()) {
      return { tokenAccount: a.pubkey, programId: TOKEN_2022_PROGRAM_ID };
    }
  }
...
// ---------------- Buy Logic ----------------
async function executePumpfunBuyFromBonding({ candidate, slippageBps = DEFAULT_SLIPPAGE_BPS } = {}) {
  ...
  // Fetch the token price from PumpFun
  ...
  if (DRY_RUN) {
    ...
    console.log("Dry Run Position Added", {
      buyLabel: `Buy ${loadActivePositions().length + 1}`,
      mint: outMintStr,
      signature: "DRY_RUN",
    });
  ...
  // Record position (SOL-based)
  const expectedOutRaw = Number(quote.outAmount || 0);

  let expectedOutUi = null;
  try {
    const decimals = await getMintDecimals(new PublicKey(outMintStr));
    expectedOutUi = expectedOutRaw / Math.pow(10, decimals);
  } catch {}

  // Dynamically determine the token account and amount bought
  const tokenAccount = (await getOrCreateTokenAccount(conn, wallet, new PublicKey(outMintStr))).tokenAccount.toBase58();
  const amountBought = expectedOutUi;  // This is the amount of the token bought (from the quote)

  // Create a position object with the relevant details
  const position = {
    mint: outMintStr,  // The mint address of the token
    tokenAccount: tokenAccount,  // The associated token account (dynamically created or found)
    amount: amountBought,  // The amount of the token bought
    buyPriceSOL: buyPriceSOL,  // The price of the token in SOL (optional)
    inputSol: Number(inputSol),  // The amount of SOL used for the purchase
    signature: sig,  // Transaction signature (if available)
    walletAddress: user.toBase58(),  // The wallet address
    openedAt: new Date().toISOString(),  // Timestamp when the position was opened
    dryRun: false,  // Set to false for real transactions
    priceSource: buyPriceSource,  // Price source (e.g., pumpfun_price)
    pumpPriceSOL: Number.isFinite(pumpPriceSOL) ? pumpPriceSOL : null,  // Pump price in SOL (optional)
    quoteOutAmount: String(quote.outAmount),  // The amount received in the quote
    expectedOutRaw: String(expectedOutRaw),  // Raw expected output (optional)
    expectedOutUi: expectedOutUi,  // Formatted expected output (optional)
  };

  // Add this position to active positions and write it to the JSON file
  const count = addActivePosition(position);

  // Optionally, you can check if the number of positions exceeds the maximum allowed
  if (count >= MAX_ENTRY) {
    await stopWatcher();  // Stop the watcher if max positions are reached
  }

  // Send a success message to Telegram with the purchase details
  await sendTelegram(
    `BUY SUCCESSFUL\nmint: ${outMintStr}\ninput: ${Number(inputSol)} SOL\namountBought: ${amountBought}\nexpectedOut: ${expectedOutUi ?? expectedOutRaw}\ntokenAccount: ${tokenAccount}\nsignature: ${sig}`
  );
