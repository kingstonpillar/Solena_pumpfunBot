...
export async function executePumpfunBuyFromBonding({
  ...
  try {
    // Replaced PumpAmmSdk logic with bonding curve logic
    const bondingCurvePublicKey = new PublicKey("6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P");  // Use the bonding curve public key

    console.log(`Buying ${amountTokens} tokens on bonding curve`);

    const instruction = new TransactionInstruction({
      programId: bondingCurvePublicKey, // Public key for bonding curve
    ...
    tx.sign(userKeypair); // Sign the transaction with the wallet
    ...
    await sendTelegram(
      `✅ Pump.fun BUY SUCCESS via Bonding Curve\n\n` +
        `Mint: ${mint.toBase58()}\n` +
