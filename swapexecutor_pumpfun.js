...
async function getOrCreateATA(connection, wallet, mint) {
  // Get the associated token address (ATA) for the mint (Pumpfun token) and wallet
  const associatedTokenAddress = await getAssociatedTokenAddress(
    mint,  // Token mint address
    wallet.publicKey  // The wallet address (the owner of the account)
  );
  ...
  if (!accountInfo) {
    ...
    const transaction = new Transaction().add(
      createAssociatedTokenAccountInstruction(
        ...
        TOKEN_PROGRAM_ID  // Correct program ID for token operations
      )
    );
