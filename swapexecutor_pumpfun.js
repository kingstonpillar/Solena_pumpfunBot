...
// Remove ONLY the ATA ix that creates the *user's* ATA (since we already created it)
buyIxs = buyIxs.filter((ix) => {
  if (!ix.programId.equals(ASSOCIATED_TOKEN_PROGRAM_ID)) return true;

  // ATA Create/CreateIdempotent ix layout: keys[1] is the associated token account address
  const ataKey = ix.keys?.[1]?.pubkey;
  if (!ataKey) return true;

  // Drop only if it targets our user's ATA
  return !ataKey.equals(ata);
});
