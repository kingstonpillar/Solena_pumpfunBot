...
// FIX: Patch all ATA CreateIdempotent ixs so the ata address matches Token-2022 derivation
if (tokenProgramId.equals(TOKEN_2022_PROGRAM_ID)) {
  ...
  for (const ix of buyIxs) {
    ...
    // If we can't read it safely, leave it unchanged
    if (!payer || !owner || !mintPk) {
      patched.push(ix);
      continue;
    }

    // IMPORTANT: for PDAs (bonding curve owners), allowOwnerOffCurve must be true
    const correctAta = await getAssociatedTokenAddress(
      mintPk,
      owner,
      true, // allowOwnerOffCurve (Pump uses PDAs)
      TOKEN_2022_PROGRAM_ID,
      ASSOCIATED_TOKEN_PROGRAM_ID
    );
    ...
    patched.push(
      createAssociatedTokenAccountIdempotentInstruction(
        payer,                   // payer
        correctAta,              // associated token account (correctly derived)
        owner,                   // owner (can be PDA)
        mintPk,                  // mint
