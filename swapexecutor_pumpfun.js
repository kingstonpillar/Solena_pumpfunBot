import { PublicKey, Keypair, Connection, Transaction, sendAndConfirmTransaction, TransactionInstruction } from '@solana/web3.js';
...
export async function executePumpfunBuyFromBonding({
  ...
  const { ata, ix: createAtaIx, created } = await getOrCreateATAIx(connection, userKeypair.publicKey, mint);
  console.log(created ? `✅ ATA created` : `✅ Using existing ATA`);
  ...
  const amountSol = BI(process.env.SOL_TO_SPEND || 1_000_000_000n);
  const amountTokens = biDiv(amountSol, buyPriceSOL);

  try {
    // Fetch recent blockhash to make the transaction valid
    const { blockhash } = await connection.getRecentBlockhash();

    console.log(`Buying ${amountTokens} tokens on bonding curve`);

    const instruction = new TransactionInstruction({
      programId: bondingCurvePublicKey, // Bonding curve public key
      data: Buffer.from([]),  // Data for bonding curve interaction
      keys: [
    ...
    const tx = new Transaction()
      .add(instruction)
      .setRecentBlockhash(blockhash);  // Set the recent blockhash
    tx.sign(userKeypair);  // Sign the transaction
...
// Create position data in JSON format
    const position = {
      mint: mint.toBase58(),
      owner: userKeypair.publicKey.toBase58(),
      tokenAccount: ata.toBase58(),
      amountTokens: biStr(amountTokens),  // Token amount is in integer format
      pumpPriceSOL: buyPriceSOL,           // Add pumpPriceSOL to position
      buyPriceSOL,                         // Add buyPriceSOL to position
      transactionSignature: signature,
      dateAdded: new Date().toISOString(),
    };

    // Log the JSON position
    console.log("Transaction Details:", JSON.stringify(position, null, 2));
    ...
    console.error("❌ Failed to execute buy:", e.message);
