...
import { fetchSolPriceUSD } from './solPriceFetcher'; // Ensure
...
const INPUT_USDC = Number(process.env.BUY_INPUT_USDC || 50);  // Amount of USDC to spend
...
async function buyTokens(mintAddressStr) {
  ...
  // Amount of USDC to spend (this is the value from .env or any defined input)
  const inputUSDC = Number(process.env.BUY_INPUT_USDC || 50); // Example: How much USDC you want to spend
  ...
  const amountBought = inputUSDC / tokenPriceUSDC;  // Amount in tokens (calculated based on USDC)
...
async function getSOLPriceInUSDC() {
  try {
    // Fetch the SOL price in USD using Pyth price feed
...
function solMint() {
  // Returning the USDC mint address on Solana
  return "9n2L4By3KKFXwtj92KrXy8K2b2kx48jBhuhehdgsLVEj";  // USDC mint address on Solana
}
...
async function executePumpfunBuyFromBonding({ candidate, slippageBps = DEFAULT_SLIPPAGE_BPS } = {}) {
  ...
  const inputUSDC = Number(process.env.BUY_INPUT_USDC || 50); // How much USDC you want to spend
  ...
  const inMint = "9n2L4By3KKFXwtj92KrXy8K2b2kx48jBhuhehdgsLVEj"; // USDC mint address
const outMint = outMintStr;  // This is the mint address for the meme token (which is dynamically passed)
  ...
  const { tokenAccount: sourceTokenAccount } = await getOrCreateTokenAccount(connection, wallet, new PublicKey(inMint));
...
try {
  ...
  await sendTelegram(`BUY ERROR: ${String(e?.message || e)}`);  // Ensure telegram is called in case of error
...
const amountBought = inputUSDC / tokenPriceUSDC; // Calculate amount bought before usage
...
const count = addActivePosition({
  ...
  // **New place to store token account**
  tokenAccount: tokenAccount.toBase58(), // Save token account address

  // **New placeholder for dynamically calculated amount bought**
  amountBought: amountBought, // Adjust dynamically based on your amount logic
});

if (count >= MAX_ENTRY) await stopWatcher();

// Send success message after all steps are completed
