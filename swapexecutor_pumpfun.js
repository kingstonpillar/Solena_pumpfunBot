...
// Example usage:
const amountSol = BigInt("1000000000"); // 1 SOL in lamports (1 SOL = 1 billion lamports)
const byteArray = bigIntToBytes(amountSol);  // Convert SOL (BigInt) to byte array

console.log(byteArray);  // This will log the byte array of the BigInt (1 SOL)
...
const amountSol = process.env.SOL_TO_SPEND
  ? BigInt(Math.floor(Number(process.env.SOL_TO_SPEND) * 1e9))  // Convert SOL to lamports and then to BigInt
  : 1_000_000_000n;  // Default to 1 SOL (1,000,000,000 lamports)
  ...
  `Buy Token Amount: ${amountTokens.toString()}\n` +
  `Tx: https://solscan.io/tx/${sig}`
);

    // After the transaction is confirmed, add the new active position to the JSON file
    const position = {
      mint: mint.toString(), // Use `mint` here instead of `mintAddress`
