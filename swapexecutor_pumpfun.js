...
export async function executePumpfunBuyFromBonding({
  ...
  try {
  // Log methods available in pumpAmmSdk
  console.log("Available methods in pumpAmmSdk:", Object.getOwnPropertyNames(PumpAmmSdk.prototype));

  // Attempt to call buyInstructions
  const result = await pumpAmmSdk.buyInstructions(
    mint,
    userKeypair,
    amountSol,
    slippageBps,
  ...
  console.log("Result from buyInstructions:", result);  // Log the result

  if (!result || !result.instructions) {
    throw new Error("❌ Failed to build PumpFun buy instructions: No instructions returned.");
  }

  const { instructions } = result;
  console.log("Pump instructions successfully built.");

  // Proceed with the transaction logic using instructions
