...
// Pump.fun bonding-curve buy executor (onchain)
// - NO Jupiter, NO Raydium
// - Wallet/SOL amount/price/slippage handled internally
// - DRY_RUN supported via .env
// - Max positions guard via .env MAX_ENTRY
// - Includes watcher start/stop logic (you requested)
// - Writes active_positions.json with mint + bondingCurve + buyPrice for tracking
...
async function ensureAtaIx(owner, mint) {
  ...
  const info = await conn.getAccountInfo(ata, COMMITMENT).catch(() => null);
...
async function getMintDecimals(mint) {
  const info = await conn.getParsedAccountInfo(mint, COMMITMENT).catch(() => null);
  const dec = info?.value?.data?.parsed?.info?.decimals;
  if (typeof dec === "number") return dec;
  return 6;
}

// PDA derivations (best-effort defaults)
...
// ---------------- MAIN EXECUTOR ----------------
// Only expects: { candidate, slippageBps? }
// Wallet, SOL amount, price, and max-positions guard are internal.
...
if (!Number.isFinite(priceSOL) || priceSOL <= 0) {
  ...
  // 3) Dry run: do not sign/send, but DO write active position (marked dryRun)
  if (DRY_RUN) {
    ...
    // after writing, enforce guard again
  ...
  // compute budget injection (optional)
  try {
  ...
  const ix = await program.methods[METHOD_NAME](
    u64Bn(minOutRaw),
    u64Bn(maxSolLamports)
  )
    .accounts(accounts)
    .instruction();
  ...
  try {
    ...
    return { signature: null, ok: false, reason: `send_failed:${e?.message || e}` };
  }

  // 5) write active position (REAL)
  const count = addActivePosition({
    buyLabel: `Buy ${loadActivePositions().length + 1}`,
    mint: candidate.mint,
...
priceSource: priceRes?.source || "unknown",
  ...
  // If max reached AFTER this buy, stop watcher
  if (count >= MAX_ENTRY) await stopWatcher();
