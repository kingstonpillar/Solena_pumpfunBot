...
export async function executePumpfunBuyFromBonding({
  ...
  if (createAtaIx) {
    ...
    console.log("✅ Creating ATA in dedicated transaction");
    ...
    console.log(`✅ ATA created: https://solscan.io/tx/${ataSig}`);

    const ataInfo = await connection.getAccountInfo(ata, "confirmed");

    if (!ataInfo) {
      throw new Error("❌ ATA creation confirmed but account not found");
    }

  } else {

    console.log(`✅ Using existing ATA ${ata.toBase58()}`);
...
// If mint is Token-2022, Pump SDK may still emit ATA CreateIdempotent ix that uses Tokenkeg.
// Patch those ATA ixs to use Token-2022.
if (tokenProgramId.equals(TOKEN_2022_PROGRAM_ID)) {
  buyIxs = buyIxs.map((ix) => {
    if (!ix.programId.equals(ASSOCIATED_TOKEN_PROGRAM_ID)) return ix;

    // Expected ATA layout for Create( Idempotent ):
    ...
    const tokenProg = ix.keys?.[5]?.pubkey;

    // If layout not as expected, do nothing
    if (!payer || !ataPk || !owner || !mintPk || !tokenProg) return ix;

    // If SDK used Tokenkeg for a Token-2022 mint, rebuild ix with Token-2022
    if (tokenProg.equals(TOKEN_PROGRAM_ID)) {
...
// Remove ONLY the ATA create instruction that targets YOUR (owner,mint,ata).
// This prevents removing other required ATA instructions.
buyIxs = buyIxs.filter((ix) => {
  if (!ix.programId.equals(ASSOCIATED_TOKEN_PROGRAM_ID)) return true;

  const ataKey = ix.keys?.[1]?.pubkey;
  const ownerKey = ix.keys?.[2]?.pubkey;
  const mintKey = ix.keys?.[3]?.pubkey;

  if (!ataKey || !ownerKey || !mintKey) return true;

  const isMyAtaCreate =
    ataKey.equals(ata) &&
    ownerKey.equals(userKeypair.publicKey) &&
    mintKey.equals(mint);

  return !isMyAtaCreate;
});
