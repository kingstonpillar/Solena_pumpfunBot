...
export async function executePumpfunBuyFromBonding({
  ...
  try {
    ...
    // Normalize bondingCurve pubkey (SDK can return different shapes)
    const bondingCurvePk =
      bondingCurveAccountInfo?.publicKey
        ? bondingCurveAccountInfo.publicKey
        : bondingCurveAccountInfo;

    if (!bondingCurvePk) throw new Error("❌ bondingCurvePk missing from fetchBuyState");

    // 1) Ensure ASSOCIATED_BONDING_CURVE (bonding curve PDA's token account) exists
    const associatedBondingCurveAta = await getAssociatedTokenAddress(
      mint,
      bondingCurvePk,
      true, // allowOwnerOffCurve (bonding curve is PDA)
      tokenProgramId, // correct token program for this mint
      ASSOCIATED_TOKEN_PROGRAM_ID
    );
