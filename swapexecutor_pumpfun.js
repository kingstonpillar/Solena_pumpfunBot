...
function bigIntToBytes(bigInt) {
  const hex = bigInt.toString(16).padStart(16, '0'); // Convert BigInt to hex string
  const byteArray = new Uint8Array(hex.match(/.{2}/g).map(byte => parseInt(byte, 16))); // Convert hex to byte array
  return byteArray;
}
