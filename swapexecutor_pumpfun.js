...
/**
 * ATA CreateIdempotent instruction data:
 *  - Associated Token Account program instruction enum:
 *    Create = 0
 *    CreateIdempotent = 1
 *  So data = Buffer.from([1])
 */
function createAtaIdempotentIx({
  ...
  return new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: tokenProgramId, isSigner: false, isWritable: false }, // CRITICAL: Tokenkeg OR Token-2022
...
export async function getOrCreateATAIx(connection, walletPubkey, mintPubkey) {
  // 1) Detect token program from mint owner
const mintInfo = await connection.getAccountInfo(mintPubkey, "confirmed");
if (!mintInfo) throw new Error(`❌ Mint account not found: ${mintPubkey.toBase58()}`);

let tokenProgramId;
if (mintInfo.owner.equals(TOKEN_2022_PROGRAM_ID)) tokenProgramId = TOKEN_2022_PROGRAM_ID;
else if (mintInfo.owner.equals(TOKEN_PROGRAM_ID)) tokenProgramId = TOKEN_PROGRAM_ID;
else throw new Error(`❌ Unknown mint owner: ${mintInfo.owner.toBase58()}`);

// 2) Derive ATA using the detected token program
...
if (!ataInfo) {
  ...
  // sanity print: THIS MUST MATCH Tokenz... for Token-2022 mints
