...
export async function Top10PCT(mintAddress) {
  try {
    ...
    // Ensure top10Data is valid and an array
    if (!top10Data || !top10Data.value || !Array.isArray(top10Data.value)) {
      console.error("Expected top10Data to be an array, but it's not:", top10Data);
      return null;
    }

    // Check if the array is empty
    if (top10Data.value.length === 0) {
      console.log("No top 10 holders found.");
      return null;
    }

    let sumTop10 = 0n;
    top10Data.value.forEach((holder, index) => {
      // Log holder data for debugging
      console.log(`Holder ${index + 1}:`, holder);

      // Ensure the 'amount' is a valid number or BigInt
      let amount = 0n;
      ...
      console.log(`Amount processed for holder ${index + 1}: ${amount}`);
