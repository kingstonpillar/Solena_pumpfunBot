...
// ✅ NEW: Top 1 percentage
export async function Top1pct(mintAddress) {
  try {
    const sup = await getTokenSupplyFromHelius(mintAddress);
    if (!sup || !sup.totalSupplyUi || sup.totalSupplyUi <= 0) return 0;

    const totalSupplyBI = BI(sup.totalSupplyUi);

    const topData = await getTop10FromHelius(mintAddress);
    const arr = Array.isArray(topData?.value) ? topData.value : [];
    if (!arr.length) return 0;

    // getTokenLargestAccounts returns uiAmount/uiAmountString and amount(string)
    const top1 = arr[0];

    // Prefer uiAmountString for human-readable, fallback to uiAmount, fallback to amount
    const top1Ui =
      top1?.uiAmountString != null
        ? top1.uiAmountString
        : top1?.uiAmount != null
        ? String(top1.uiAmount)
        : top1?.amount != null
        ? String(top1.amount)
        : "0";

    const top1BI = BI(top1Ui);

    const pctStr = biPct(top1BI, totalSupplyBI); // usually returns string like "51" or "51.23"
    const pctNum = Number(pctStr);

    if (!Number.isFinite(pctNum)) return 0;

    const safePct = Math.max(0, Math.min(100, pctNum));
    return safePct;
  } catch (e) {
