...
const MAX_TX_AGE_SEC = 30; // <-- Only process transactions <= 30 seconds old
    ...
    try {
      ...
      for (let slot = lastSlot + 1; slot <= endSlot; slot++) {
        const block = await rpcLimited("getBlock", (c) =>
          c.getBlock(slot, { commitment: "confirmed", transactionDetails: "full", maxSupportedTransactionVersion: 1 })
        );
        if (!block?.transactions) continue;

        for (const tx of block.transactions) {
          const sig = tx.transaction.signatures[0];
          const txTime = block.blockTime; // seconds
          if (!sig || seenSigs.has(sig)) continue;
          if (!txTime || nowSec - txTime > MAX_TX_AGE_SEC) continue; // <-- skip old txs

          seenSigs.add(sig);

          // Scan instructions for PumpSwap migration program
          const touchesProgram = tx.transaction.message.instructions.some(ix =>
            ix.programId.equals(PUMPSWAP_MIGRATION_PROGRAM_ID)
          );
          ...
          // Limit tracked to avoid memory bloat
