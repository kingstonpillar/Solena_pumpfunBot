...
async function discoverNewTokens() {
  ...
  for (const pair of pairs) {
    ...
    let migrated = false;
    try {
      migrated = await checkPumpMigration(pairAddress); // checkPumpMigration should internally use your RPC limiter
    } catch (err) {
...
console.log(`[+] DexScreener pumpswap tracker started | interval=${SCAN_INTERVAL_MS}ms`);