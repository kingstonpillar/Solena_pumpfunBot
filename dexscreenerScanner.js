...
async function fetchFreshPumpSwapPairs() {
  ...
  const freshPairs = pairs.filter(p => {
    ...
    // Must have valid createdAt
    if (!p.createdAt) return false;

    const createdAt = new Date(p.createdAt).getTime();
    if (Number.isNaN(createdAt)) return false;

    // Token must be within TTL
    if (now - createdAt > CANDIDATE_TTL_MS) return false;

    return true;
  });

  console.log(`[Scanner] Fresh PumpSwap candidates found: ${freshPairs.length}`);
  return freshPairs;
}
