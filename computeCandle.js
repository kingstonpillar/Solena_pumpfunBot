...
export async function computeCandle(
  ...
  try {
    ...
    // ---------- Get current price ----------
    const priceObj = await getPumpFunPriceOnce({ mint: tokenMint });
    if (
      !priceObj ||
    ...
    if (color === "green") {
      console.log(
        `[CANDLE ✅ GREEN] ${tokenMint} | momentum=${momentum.toFixed(6)} | momentumPct=${momentumPct.toFixed(2)}% | required=${minMovement.toFixed(6)} | requiredPctOfRange=${minMomentumPercent}% | range=${range.toFixed(6)}`
      );
    } else {
      console.log(
        `[CANDLE ❌ NO GREEN] ${tokenMint} | momentum=${momentum.toFixed(6)} | momentumPct=${momentumPct.toFixed(2)}% | required=${minMovement.toFixed(6)} | requiredPctOfRange=${minMomentumPercent}% | range=${range.toFixed(6)}`
      );
    }
