...
function isExpiredCandidate(candidate) {
  ...
  const candidateTime = Date.parse(candidate.firstSeenAt); // Assuming candidate.firstSeenAt is a timestamp string
...
async function runBuyCallerOnce() {
  ...
  for (const c of candidates) {
    ...
    if (!mint || processedSet.has(mint)) {
      logEvent(`Skipping already processed mint: ${mint}`, 'INFO');
      continue; // Skip if already processed
    }

    if (isExpiredCandidate(c)) {
      logEvent(`Skipping expired candidate: ${mint}`, 'INFO');
      continue; // Skip if expired
    }

    logEvent(`[BUY-CHECK] mint=${mint} curveSol=${c?.solLiquidity ?? c?.curveSol ?? '?'}`, 'DEBUG');
    ...
    if (!mintProg?.ok) {
      logEvent(`mint program owner check failed for mint=${mint}: ${mintProg?.reason}`, 'FAIL');
      nextCandidates.push(c);
      continue;
    }
    ...
    if (mintProg.program === 'spl-token-2022') {
      ...
      try {
        ...
        logEvent(`[ERROR] token-2022 extension gate failed for ${mint}: ${String(e?.message || e)}`, 'ERROR');
        nextCandidates.push(c);
        continue;
      }

      if (!extGate?.ok) {
        logEvent(`[FAIL] token-2022 extensions for mint=${mint}: ${extGate?.reason || 'denied'}`, 'FAIL');
        continue;
      }
    }
    ...
    if (!creatorRes?.safe) {
      if (creatorRes.rpcFailed) {
        logEvent(`creator check failed for mint=${mint}, retrying later: ${String(creatorRes.reasons || '')}`, 'RPC');
        nextCandidates.push(c);
        continue;
      }
      logEvent(`creator security check failed for mint=${mint}: ${creatorRes?.reasons || creatorRes}`, 'FAIL');
      continue; // Skip if creator check fails
    }
    ...
    if (!tokenRes?.safe) {
      if (tokenRes.rpcFailed) {
        logEvent(`token security check failed for mint=${mint}, retrying later: ${String(tokenRes.reasons || '')}`, 'RPC');
        nextCandidates.push(c);
        continue;
      }
      logEvent(`token security check failed for mint=${mint}: ${tokenRes?.reasons || tokenRes}`, 'FAIL');
      continue; // Skip if token security fails
    }
    ...
    if (!sig) {
      if (buyRes.rpcFailed) {
        logEvent(`buy call failed for mint=${mint}, retrying later: ${String(buyRes.error || '')}`, 'RPC');
        nextCandidates.push(c);
        continue;
      }
      logEvent(`buy failed for mint=${mint}: ${String(buyRes?.error || '')}`, 'FAIL');
      continue; // Skip if buy failed
    }

    logEvent(`[BOUGHT] mint=${mint} tx=${sig}`, 'SUCCESS');
    buysDone += 1;
