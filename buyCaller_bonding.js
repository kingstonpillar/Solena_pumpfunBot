...
export async function resumeWatcherIfBelowMax() {
  ...
  let count = active.length; // Initial count based on the current active positions

  // Increment the count for each new successful purchase
  if (count < MAX_ENTRY) {
    count += 1;  // Simulating the purchase of a new token and incrementing the count
  }

  if (count >= MAX_ENTRY) {
    await stopWatcher();  // Stop buying if max entries are reached
    return { ok: false, reason: 'max_entry_reached', count: count };
  }
