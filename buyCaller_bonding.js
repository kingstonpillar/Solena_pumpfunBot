...
async function runBuyCallerOnce() {
  ...
  for (const c of candidates) {
    ...
    // Log: Security check for Mint Program Owner
    let mintProg;
    try {
      mintProg = await getMintProgramOwner(mint);
      console.log(`[SECURITY CHECK] mint program owner: ${mintProg?.program}`);
    } catch (e) {
    ...
    // Log: Security check for SPL Token 2022 Extensions
    if (mintProg.program === 'spl-token-2022') {
      ...
      try {
        ...
        console.log(`[SECURITY CHECK] token-2022 extension gate result: ${extGate?.ok ? 'Passed' : 'Denied'}`);
      } catch (e) {
    ...
    console.log(`[SECURITY CHECK] creator safety check result: ${creatorRes?.safe ? 'Passed' : 'Failed'}`);
    ...
    console.log(`[SECURITY CHECK] token security check result: ${tokenRes?.safe ? 'Passed' : 'Failed'}`);
  ...
  // Save processed list and update candidates
