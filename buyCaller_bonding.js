...
// Updates applied:
// (1) Removed top holder logic (holder check).
...
function touchesProgram(tx, programIdBase58) {
  const msg = tx?.transaction?.message;
  if (!msg) return false;

  const keys = getMessageKeys(msg).map((k) =>
    k?.pubkey?.toBase58 ? k.pubkey.toBase58() : k?.toBase58 ? k.toBase58() : String(k)
  );

  return getCompiledInstructions(msg).some((ix) => {
    const idx = ix?.programIdIndex;
    if (typeof idx !== "number") return false;
    return keys[idx] === programIdBase58;
  });
}
...
const isClassic = ownerStr === TOKEN_PROGRAM_ID.toBase58();
...
// ---------------- core metrics ----------------
async function getBuysInWindow({ mintStr, bondingCurveStr }, windowStartSec, sigLimit = SIG_LIMIT) {
  const mintPub = toPubkey(mintStr);
  const curvePub = bondingCurveStr ? toPubkey(bondingCurveStr) : null;

  // Prefer bonding curve stream for pump buys
  const sigAddress = curvePub || mintPub;
  ...
  const sigs = await rpcLimited(
    `getSignaturesForAddress(${curvePub ? "curve" : "mint"})`,
    (c) => c.getSignaturesForAddress(sigAddress, { limit: Math.min(sigLimit, 1000) })
  );
  ...
  for (const s of sigs) {
    ...
    // Only count pump-related txs (prevents mint/curve noise)
    if (!touchesProgram(tx, pump)) continue;

    const payer = getFeePayer(tx);
    if (payer) buyers.add(payer);

    // Count a "buy" as any pump tx in the window.
    // This removes false negatives from token-balance parsing quirks.
    buyCount += 1;

    // Token volume is best-effort: sum positive deltas for the mint across accounts.
    ...
    for (const p of post) {
      ...
      if (delta > 0) txDeltaSum += delta;
    }

    if (txDeltaSum > 0) {
      tokenBuyVolume += txDeltaSum;
      totalDelta += txDeltaSum;

      // Attribute to payer (more consistent than token balance owner on pump flows)
      if (payer) deltasByOwner[payer] = (deltasByOwner[payer] || 0) + txDeltaSum;
    }
...
const tokenRes = await checkTokenSecurity({
  ...
  reasons: String(e?.message || e || "token_security_failed"),
    ...
    const stats = await getBuysInWindow(
  { mintStr: mint, bondingCurveStr: c?.bondingCurve },
  windowStartSec,
  SIG_LIMIT
).catch((e) => {
...
if (stats?.rpcError) {
  ...
  continue;
    ...
    // -------- BUY --------
if (BUY_DRY_RUN) { // Opening bracket for dry run check
  console.log(`[BUY-DRY-RUN] buy skipped for ${mint}`);
  buysDone += 1;
  processedList = markProcessedMint(processedList, mint);
  processedSet.add(mint);
  continue; // Skip the buy process in dry run mode
} // Closing bracket for dry run check

try { // Opening try block for buy attempt
  // Attempt to buy the token after all checks
  ...
  if (!sig) { // If the signature is not present, log failure and retry
    if (buyRes.rpcFailed) {
  ...
  } // Closing if block for checking signature

  // If the buy is successful, log the transaction and proceed
  console.log(`[BOUGHT] ${mint} tx=${sig}`);
  buysDone += 1;

  // Mark this mint as processed and add to the set
  processedList = markProcessedMint(processedList, mint);
  processedSet.add(mint);

} catch (e) { // Opening catch block for unexpected errors
  // Catch unexpected errors and log them
  console.log("[ERROR] Unexpected error during buy process:", e);
  continue; // Skip this candidate and continue with the next one
} // Closing catch block

// After completing all operations, save the controlled lists
