...
async function runBuyCallerOnce() {
  ...
  // Only log critical events like when candidates are processed or failed
  if (allCandidates.length === 0) {
    console.log('[INFO] No candidates found.');
    return;
  }

  // Prune expired candidates and sort by most recent
  const candidates = pruneExpiredCandidates(allCandidates);
  candidates.sort((a, b) => {
  ...
  for (const c of candidates) {
    ...
    // Only log if something goes wrong with the security check
    let mintProg;
    try {
      mintProg = await getMintProgramOwner(mint);
    } catch (e) {
      console.log(`[ERROR] getMintProgramOwner failed for ${mint}:`, String(e?.message || e));
      nextCandidates.push(c);
      continue;
    }

    if (!mintProg?.ok) {
      console.log(`[ERROR] mint program owner check failed for ${mint}: ${mintProg?.reason}`);
      continue;
    }
    ...
    if (mintProg.program === 'spl-token-2022') {
      ...
      try {
        ...
        console.log(`[ERROR] token-2022 extension gate failed for ${mint}:`, String(e?.message || e));
        nextCandidates.push(c);
        continue;
      }

      if (!extGate?.ok) {
        console.log(`[ERROR] token-2022 extension failed for ${mint}: ${extGate?.reason || 'denied'}`);
        continue;
      }
    }
    ...
    if (!creatorRes?.safe) {
      if (creatorRes.rpcFailed) {
        console.log(`[ERROR] creator check failed for ${mint}, will retry later:`, String(creatorRes.reasons || ''));
        nextCandidates.push(c);
        continue;
      }
      console.log(`[ERROR] creator security failed for ${mint}:`, creatorRes?.reasons || creatorRes);
      continue;
    }
    ...
    if (!tokenRes?.safe) {
      if (tokenRes.rpcFailed) {
        console.log(`[ERROR] token security failed for ${mint}, will retry later:`, String(tokenRes.reasons || ''));
        nextCandidates.push(c);
        continue;
      }
      console.log(`[ERROR] token security failed for ${mint}:`, tokenRes?.reasons || tokenRes);
      continue;
    }
    ...
    if (!sig) {
      if (buyRes.rpcFailed) {
        console.log(`[ERROR] buy call failed for ${mint}, will retry later:`, String(buyRes.error || ''));
        nextCandidates.push(c);
        continue;
      }
      console.log(`[ERROR] buy failed for ${mint}:`, String(buyRes?.error || ''));
      continue;
    }
