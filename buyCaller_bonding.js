...
async function getTopHolderPct(mintStr) {
  ...
  //  Use getTokenSupply (raw amount)
  const supplyResp = await rpcLimited("getTokenSupply(mint)", (c) =>
    c.getTokenSupply(mintPub, { commitment: COMMITMENT })
  );
  ...
  if (!top?.amount) {
    return { ok: false, pct: 100, reason: "no_largest_accounts" };
  }

  const topAmt = BigInt(top.amount || "0");
  if (topAmt <= 0n) {
    return { ok: false, pct: 100, reason: "top_amount_invalid" };
  }

  //  precise percent with 2 decimal places
  const pct = Number((topAmt * 10_000n) / supply) / 100;
