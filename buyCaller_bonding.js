...
async function runBuyTick(label) {
  ...
  try {
    ...
    // Option A: HARD stop when max entries hit
 if (!gate?.ok) {
  if (buyTimer) {
    clearInterval(buyTimer);
    buyTimer = null;
  }
  console.log("[buyCaller] stopped", { reason: `max_entry_reached:${gate?.count ?? "unknown"}` });
  return;
}
...
function extractCreatorWallets(creatorRes) {
  ...
  const arr =
    creatorRes?.relatedWallets || creatorRes?.wallets || creatorRes?.details?.wallets;
...
export async function runBuyCallerOnce() {
  ...
  for (const c of candidates) {
    ...
    // -------- Mint program owner gate (NEW, no logic change to rest) --------
