...
import { executeAmmMigrationBuy } from './swaoexecutorAMM_pumpswap.js';
...
// ---------------- PROCESSED MINTS UTILITY ----------------

function markProcessedMint(processedList, mintStr) {
  if (!mintStr) return processedList;

  // Add new mint if it doesn't exist
  if (!processedList.includes(mintStr)) {
    processedList.push(mintStr);
    console.log(`[PROCESSED_MINTS] Added mint: ${mintStr}`);
  } else {
    console.log(`[PROCESSED_MINTS] Mint already in list: ${mintStr}`);
  }

  // Trim oldest mints if list exceeds MAX_BUYS
  if (processedList.length > MAX_BUYS) {
    const excess = processedList.length - MAX_BUYS;
    const removed = processedList.slice(0, excess);
    processedList = processedList.slice(-MAX_BUYS);

    console.log(`[PROCESSED_MINTS] Trimmed oldest mints: ${removed.join(', ')}`);
  }

  return processedList;
}

// Load processed mints (tokens already bought)
...
// Save processed mints to JSON
function saveProcessedList(processedList) {
  try {
    fs.writeFileSync(PROCESSED_MINTS_FILE, JSON.stringify(processedList, null, 2), 'utf8');
  } catch (err) {
    console.error('Error saving processed mints:', err);
  }
}
...
async function runBuyCallerOnce() {
  ...
  for (const c of candidates) {
    ...
    const buyRes = await executeAmmMigrationBuy({
  candidate: c,
