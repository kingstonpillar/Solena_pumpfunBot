...
const BUY_TOKEN_FILE = path.resolve(
  process.env.BUY_TOKEN_FILE || "./buyToken.json"
);
...
const BUY_TOKEN_MAP = loadMapFromFile(BUY_TOKEN_FILE);
...
function persistBuyTokenMap() {
  saveMapToFile(BUY_TOKEN_MAP, BUY_TOKEN_FILE);
}

function getBuyTokenState(mint) {
  const cleanMint = String(mint || "").trim();
  if (!cleanMint) return null;
  return BUY_TOKEN_MAP.get(cleanMint) || null;
}

function markTokenBuying(mint, extra = {}) {
  const cleanMint = String(mint || "").trim();
  if (!cleanMint) return;

  BUY_TOKEN_MAP.set(cleanMint, {
    status: "buying",
    updatedAt: Date.now(),
    ...extra,
  });
...
function markTokenBought(mint, extra = {}) {
  const cleanMint = String(mint || "").trim();
  if (!cleanMint) return;
...
function clearBuyTokenState(mint) {
  const cleanMint = String(mint || "").trim();
  if (!cleanMint) return false;

  const deleted = BUY_TOKEN_MAP.delete(cleanMint);
  persistBuyTokenMap();
  return deleted;
}
...
async function processMint(mint) {
  ...
  const existingBuyState = getBuyTokenState(mint);

  if (existingBuyState?.status === "buying") {
    console.log("[BUY_TOKEN_SKIP]", {
      mint,
      reason: "already_marked_buying",
      state: existingBuyState,
    });
    return false;
  }

  if (existingBuyState?.status === "bought") {
    console.log("[BUY_TOKEN_SKIP]", {
      mint,
      reason: "already_marked_bought",
      state: existingBuyState,
    });
    removeMintFully(mint, "already_bought");
    return false;
  }
  ...
  try {
    ...
    markTokenBuying(mint, {
      stage: "pre_buy",
      firstSeenAt: FIRST_SEEN_MAP.get(mint) || now,
    });
    ...
    markTokenBought(mint, {
      signature: buyRes.signature,
      boughtAt: Date.now(),
      pool: String(poolPk?.toBase58?.() || ""),
    });
