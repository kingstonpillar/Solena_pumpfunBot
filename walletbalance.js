...
async function buildPositionsReport(walletTokens) {
  ...
  for (const pos of sliced) {
    ...
    lines.push(
  `• ${mint}\n` +
  `  amount: ${fmt(tokenAmt, 4)} | entry: ${entryPrice ? fmt(entryPrice, 10) : "n/a"} | now: ${priceStr}\n` +
...
const body = lines
  .map((line) => line.replace(/[*`]/g, "")) // strip any leftover formatting
  .join("\n\n");

const footer = `\n\nTime: ${nowStr()}`;

const msg = header + totals + body + footer;
...
async function runWalletTick(label) {
  if (walletTickRunning) {
    console.log("[walletbalance] tick skipped, still running", { label });
    return;
  }
  ...
  try {
    ...
    console.log("[walletbalance] tick finished", {
      label,
      durationSec: ((Date.now() - started) / 1000).toFixed(2),
    });
...
export function startWalletReporter() {
  ...
  walletTimer = setInterval(() => {
    console.log("[walletbalance] interval fired", {
