...
async function derivePumpfunCreatorFast(mintPub) {
  ...
  for (const s of sigInfos) {
    ...
    // fee payer = accountKeys[0] (best effort)
...
export async function verifyCreatorSafetyPumpfun(mintOrRecord) {
  ...
  // 1) Normalize resolver input (MUST be a string, never an object)
  let resolverInput =
    typeof mintOrRecord === "string"
      ? mintOrRecord
      : (mintOrRecord?.mint?.toBase58 ? mintOrRecord.mint.toBase58() : mintOrRecord?.mint) ||
        mintOrRecord?.bondingCurve ||
        mintOrRecord?.curve ||
        mintOrRecord?.pair ||
        mintOrRecord?.pairAddress ||
        ""; // DO NOT fall back to the whole object

  resolverInput = String(resolverInput || "").trim();

  if (!resolverInput) {
    return {
      safe: false,
      score: 0,
      reasons: ["bad_resolver_input"],
      creator: null,
      details: { input: mintOrRecord, resolverInput },
    };
  }

  // 2) Resolve to real 32-byte mint using RPC failover
  const resolved = await rpcLimited("resolveMintOnChain", (c) =>
    resolveMintOnChain(c, resolverInput, COMMITMENT)
  ).catch((e) => ({
    ok: false,
  ...
  if (!resolved?.ok || !resolved.mint) {
    return {
      ...
      creator: null,
      details: { input: resolverInput, resolver: resolved },
    };
  }

  const mintStr = String(resolved.mint).trim();
  ...
  try {
    ...
    return {
      ...
      creator: null,
      details: { mintStr, resolver: resolved },
    };
  }
  ...
  if (!derived.creator) {
    // if we never saw Pump program, it is not a Pump token (no false negative)
    if (derived.sawPump) {
