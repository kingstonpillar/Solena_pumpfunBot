...
// NEW: if we cannot derive creator, do NOT hard fail. Apply a penalty only.
const CREATOR_UNKNOWN_PENALTY = Number(process.env.CREATOR_UNKNOWN_PENALTY || 25);

// ---------------- Scan limits ----------------
const MINT_SIG_SCAN_LIMIT = Number(process.env.MINT_SIG_SCAN_LIMIT || 120);
const DEV_SIG_SCAN_LIMIT = Number(process.env.DEV_SIG_SCAN_LIMIT || 500);

// NEW: optional paging so you can scan deeper when creator is hard to find
const MINT_SCAN_HARD_CAP = Number(process.env.MINT_SCAN_HARD_CAP || 2000);
...
// ---------------- Derive creator wallet (improved, paged, safer pick) ----------------
async function derivePumpfunCreator(mintPub) {
  let before = undefined;
  let scanned = 0;

  while (scanned < MINT_SCAN_HARD_CAP) {
    const sigInfos = await rpcLimited("getSignaturesForAddress(mint)", (conn) =>
      conn.getSignaturesForAddress(mintPub, { limit: MINT_SIG_SCAN_LIMIT, before })
    ).catch(() => []);
    ...
    for (const s of sigInfos) {
      ...
      // Prefer any signer if present, else fallback to fee payer (accountKeys[0])
      const keys = tx.transaction?.message?.accountKeys || [];

      const signerKey = keys.find((k) => k?.signer);
      const signer =
        signerKey?.pubkey?.toBase58?.() || signerKey?.toBase58?.() || null;

      const payerKey = keys[0];
      const payer =
        payerKey?.pubkey?.toBase58?.() || payerKey?.toBase58?.() || null;

      const creator = signer || payer;

      if (creator) {
        return { creator, signature: sig, slot: tx.slot, blockTime: tx.blockTime || null, scanned };
      }
    }
  }

  return { creator: null, signature: null, slot: null, blockTime: null, scanned };
}
...
async function analyzeCreatorWallet(creatorAddress) {
  ...
  try {
    const bal = await rpcLimited("getBalance(creator)", (conn) => conn.getBalance(creatorPub)).catch(
      () => null
    );
...
// ---------------- Exported API ----------------
// IMPORTANT CHANGE:
// - If creator cannot be derived, do NOT hard fail.
// - Apply penalty + return "creator_unknown" so your other scanners can decide.
export async function verifyCreatorSafetyPumpfun(mint) {
  ...
  // ✅ NEW BEHAVIOR: do not hard fail if creator can't be derived
  if (!derived.creator) {
    score = Math.max(0, score - CREATOR_UNKNOWN_PENALTY);
    reasons.push("creator_unknown");

    return {
      safe: score >= MIN_CREATOR_SCORE, // pass/fail based on remaining score
      score,
      reasons,
      creator: null,
      details: { derived, token2022: t22 },
    };
  }
