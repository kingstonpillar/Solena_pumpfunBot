...
// Try as-is first. Only strip "pump" if as-is fails.
function normalizeMint(m) {
  const raw = stripInvisible(m);

  const asIs = tryPk(raw);
  if (asIs) return asIs;

  if (raw.toLowerCase().endsWith("pump")) {
    const stripped = stripInvisible(raw.slice(0, -4));
    return tryPk(stripped);
  }
...
async function evaluateOnePosition({ pos }) {
  ...
  if (!mint) {
    console.log("[SKIP] invalid mint in active_positions:", rawMint);
    return null;
  }
  ...
  try {
    const pr = await getPumpFunPriceOnce({ mint, migration: pos?.migration || null });
  ...
  const decision = decideTop1Guard(top1Pct, profitPct);
