...
async function rpcTryAll(opName, fn) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      lastErr = e;
      // Keep trying next RPC regardless of retryability (your preference).
      continue;
    }
  }
...
// Normalize account.data variants into a Buffer (or null)
function toU8(data) {
  if (!data) return null;
  ...
  if (data?.data && Array.isArray(data.data) && typeof data.data[0] === "string") {
    return Buffer.from(data.data[0], "base64");
  }
...
// Token account base layout (both classic + token-2022):
// mint: 0..32, owner: 32..64, amount: 64..72
const AMOUNT_OFFSET = 64;

async function getTop10Pct(c, mintPub, commitment = "confirmed", isToken2022 = false) {
  // supply (works for both token programs)
  const supplyResp = await c.getTokenSupply(mintPub, { commitment });
  const supply = BigInt(supplyResp?.value?.amount || "0");
  if (supply <= 0n) return { ok: false, pct: null, reason: "supply_zero" };

  // 1) fast path
  try {
    const largestResp = await c.getTokenLargestAccounts(mintPub, { commitment });
    const list = Array.isArray(largestResp?.value) ? largestResp.value : [];

    if (list.length) {
      let sumTop10 = 0n;
      for (let i = 0; i < Math.min(10, list.length); i++) {
        sumTop10 += BigInt(list[i]?.amount || "0");
      }
      const pct = Number((sumTop10 * 10_000n) / supply) / 100;
      return { ok: true, pct, reason: "largest_accounts" };
    }
  } catch {
    // swallow and fallback below
  }

  // 2) fallback only for token-2022
  if (!isToken2022) {
    return { ok: false, pct: null, reason: "no_largest_accounts" };
  }

  const resp = await c.getTokenAccountsByMint(
    mintPub,
  ...
  const accs = Array.isArray(resp?.value) ? resp.value : [];
  if (!accs.length) return { ok: false, pct: null, reason: "no_token_accounts_2022" };

  // keep top10 amounts without sorting whole array
  const top = [];
  for (const a of accs) {
    const raw = toU8(a?.account?.data);
    const amt = raw ? readU64LE(raw, 0) : null; // sliced at AMOUNT_OFFSET
    if (amt == null || amt <= 0n) continue;

    top.push(amt);
    top.sort((x, y) => (y > x ? 1 : y < x ? -1 : 0));
    if (top.length > 10) top.length = 10;
  }

  if (!top.length) return { ok: false, pct: null, reason: "top10_all_zero" };

  let sumTop10 = 0n;
  for (const x of top) sumTop10 += x;

  const pct = Number((sumTop10 * 10_000n) / supply) / 100;
  return { ok: true, pct, reason: "token_accounts_scan_2022" };
}
