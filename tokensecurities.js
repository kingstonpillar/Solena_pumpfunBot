...
// Function to calculate the score based on the percentage
function calculateScore(percentage) {
  return percentage < 27 ? 20 : 0;  // Award 20 if percentage is below 27, otherwise award 0
}
...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  // Step 1: Fetch the dynamic percentage from Top10PCT function
  const top10Pct = await Top10PCT(mintStr);  // Fetch dynamic top 10 percentage

  // Step 2: Check if the result is a string with a percentage or number
  let numericPercentage;

  if (typeof top10Pct === 'string') {
    // If it's a string like "17%", remove the '%' sign
    numericPercentage = parseFloat(top10Pct.replace('%', '').trim());
  } else if (typeof top10Pct === 'number') {
    // If it's already a number (e.g., 17)
    numericPercentage = top10Pct;
  } else {
    // If the value is neither a number nor a valid string, handle the error
    console.log("Error: The top 10 percentage is invalid or not a number.");
    numericPercentage = 0;  // Default value if something went wrong
  }

  // Step 3: Calculate the score based on the top 10 percentage
  const top10Score = calculateScore(numericPercentage);  // Use the numeric percentage for score calculation

  // Step 4: Add the score from top 10 holders' percentage
  score += top10Score;

  // Final adjustments to score, ensure it doesn't exceed 100
  if (score > 100) score = 100;

  // Output the result (Top 10 Percentage and Score)
  console.log(`Top 10 holders own: ${top10Pct}% of the total supply.`);  // Keep the '%' in the output
