...
function extractMintAuthorities(parsedResp) {
  ...
  const isInitialized = safeBool(info?.isInitialized);
...
async function top10ConcentrationBigInt(mintPub) {
  ...
  // pct as Number is fine here (ratio only)
  const pct = Number(topSum) / Number(supplyAmt);

  return { ok: true, pct, supplyAmount: supplyAmt.toString(), top10Amount: topSum.toString(), decimals };
}

// ---------------- PUBLIC API ----------------
export async function verifyTokenSecurity(mintStr) {
  const reasons = [];
  let score = 0;
  ...
  const { parsed, raw } = await getMintParsedAndRaw(mintPub).catch((e) => {
    return { parsed: null, raw: null, error: String(e?.message || e) };
  });
  ...
  if (!isTokenProgram && !isToken2022) {
    return {
      ...
      details: { owner: ownerStr, rpcUsed: activeRpcUrl },
    };
  }
  ...
  if (!isInitialized) {
    return {
      ...
      details: { rpcUsed: activeRpcUrl, decimals },
    };
  }
  ...
  if (isToken2022) {
    ...
    if (r.dangerous) {
      return {
        ...
        details: { token2022: r, rpcUsed: activeRpcUrl },
      };
    }

    // If you want to allow Token-2022 without dangerous extensions, keep this as pass.
    // If you want to block ALL Token-2022, return unsafe here instead.
    score += 10;
    reasons.push("token2022_no_dangerous_extensions");
  }

  // 4) Optional: distribution (top10)
