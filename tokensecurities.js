...
let t10 = null;
try {
  t10 = await Top10PCT(mintPub);  // Using the Top10PCT function here
  console.log('Top 10 Response:', t10);  // Log the full response for debugging
} catch (error) {
  console.error("Error fetching top 10:", error);
  t10 = null;
}

// Check if Top 10 data is valid and process the result
if (t10 == null || t10.pct == null) {
  // If t10 is null or pct is not available, report an issue with data fetching
  console.error(`Top 10 RPC failed with reason: ${t10?.reason || "rpc_fail"}`);
  reasons.push(`top10_unknown_${t10?.reason || "rpc_fail"}`);
  details.distribution = { reason: t10?.reason || "rpc_fail" };
} else {
  // If pct is valid, check its value against the threshold
  details.distribution = { top10Pct: t10.pct, reason: t10.reason };

  // Log top 10 percentage to check if it's valid
  console.log('Top 10 percentage:', t10.pct);

  // If the percentage exceeds the threshold (0.27), it's considered unsafe (failure)
  if (t10.pct > 0.27) { // MAX top 10 percent threshold
    ...
    return {
      ...
      score: 0,
      reasons: [
        `top10_hard_fail_${t10.pct.toFixed(2)}_gt_0.27`,
      ],
      details,
    };
  }

  // If t10.pct <= 0.27, it is considered safe and valid
  score += 15;
