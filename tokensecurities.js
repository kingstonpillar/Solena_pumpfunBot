...
let top10PctRaw = 0;
try {
  top10PctRaw = await Top10PCT(mintStr);
} catch (e) {
  top10PctRaw = 0;
  reasons.push("top10_fetch_failed");
}

// Normalize safely
let numericPercentage = 0;

if (typeof top10PctRaw === "string") {
  numericPercentage = parseFloat(top10PctRaw.replace("%", "").trim());
} else if (typeof top10PctRaw === "number") {
  numericPercentage = top10PctRaw;
}
...
details.t10 = {
  raw: top10PctRaw,
  pct: numericPercentage,
};

// Apply scoring
if (numericPercentage >= 40) {
  score -= 25;
...
// Logs (this is what you are missing)
console.log(`Top 10 holders own: ${numericPercentage}% of the total supply.`);
