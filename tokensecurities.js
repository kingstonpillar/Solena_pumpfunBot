...
const MIN_BONDING_SOL = Number(process.env.MIN_BONDING_SOL || 0); // example 0.2
...
async function checkLiquidityState(mintPub, migration) {
  ...
  // A) Bonding curve proof: curve PDA exists AND has SOL above floor
  ...
  // B) Migrated pool proof: vault reserves exist (WSOL + token)
...
async function findPumpfunAndMigration(mintPub) {
  ...
  while (scanned < HARD_CAP && (!isPumpFun || !migration)) {
    ...
    if (scanned % 120 === 0) {
      console.log(`...scan progress: ${scanned}/${HARD_CAP} sigs (before=${before})`);
    }
...
export async function verifyTokenSecurity(mint) {
  ...
  log("\n[1] scanning for pumpfun origin + migration...");
  const { isPumpFun, migration, lpMint, scanned } = await findPumpfunAndMigration(mintPub);
  log("[1] scan done:", { isPumpFun, migration, lpMint, scanned });

  // hard requirement
  if (!isPumpFun) {
    log("[FAIL] not pumpfun origin");
    return {
      safe: false,
      score: 0,
      reasons: ["Not a Pump.fun mint"],
      details: { isPumpFun, scanned, rpcUsed: activeRpcUrl },
    };
  }
  score += 20;
  reasons.push("pumpfun_origin");

  // 2) Liquidity state label
  if (migration) {
    score += 20;
  ...
  if (mintAuthority) {
    log("[FAIL] mint authority present:", mintAuthority);
    return {
      safe: false,
      score: 0,
      reasons: [`mint authority NOT renounced -> ${mintAuthority}`],
      details: { mintAuthority, freezeAuthority, isPumpFun, migration, scanned, rpcUsed: activeRpcUrl },
    };
  }
  score += 15;
  ...
  if (freezeAuthority) {
    log("[FAIL] freeze authority present:", freezeAuthority);
    return {
      safe: false,
      score: 0,
  ...
  if (!hp.ok) {
    log("[FAIL] honeypot gate:", hp.reason);
    return {
      safe: false,
      score: 0,
      reasons: [`honeypot_risk_gate_fail (${hp.reason})`],
      details: { isPumpFun, migration, lpMint, scanned, honeypotGate: hp, rpcUsed: activeRpcUrl },
    };
  }
  score += 30;
