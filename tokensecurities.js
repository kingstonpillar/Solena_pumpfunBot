...
// Pump.fun origin via API first, on-chain fallback second.
// IMPORTANT: do NOT treat RPC failures as "no signatures".
...
const MIN_BONDING_SOL = Number(process.env.MIN_BONDING_SOL || 0);
...
async function checkLiquidityState(mintPub, migration) {
  ...
  const curveAcc = await rpcLimited("getAccountInfo(bondingCurve)", (c) =>
    c.getAccountInfo(curvePda, { commitment: COMMITMENT })
  ).catch(() => null);
  ...
  if (curveAcc) {
    ...
    if (Number.isFinite(sol) && sol > MIN_BONDING_SOL) {
      return { ok: true, venue: "bonding_curve", curve: curvePda.toBase58(), solLiquidity: +sol.toFixed(6) };
    }
  }

  if (migration?.signature) {
    const vaults = await discoverVaultsFromMigrationTx(migration.signature, mintStr);
    if (!vaults) return { ok: false, venue: migration.venue || "migrated", reason: "vault_discovery_failed", migration };

    const wsol = await getUiBal(vaults.wsolVault);
    const tok = await getUiBal(vaults.tokenVault);

    if (wsol > 0 && tok > 0) {
      return { ok: true, venue: migration.venue || "migrated", migration, reserves: { wsol, token: tok }, vaults };
    }

    return { ok: false, venue: migration.venue || "migrated", reason: "zero_reserves", reserves: { wsol, token: tok }, vaults, migration };
  }
...
// ---------------- FIXED SCANNER ----------------
// Key change: do NOT swallow RPC errors as "no signatures".
async function findPumpfunAndMigrationOnchain(mintPub) {
  const mintStr = mintPub.toBase58();
  ...
  while (scanned < HARD_CAP && (!isPumpFun || !migration)) {
    ...
    if (scanned % 120 === 0) log(`...scan progress: ${scanned}/${HARD_CAP} sigs (before=${before})`);
    ...
    for (const s of sigInfos) {
      ...
      if ((touchesRaydium || touchesPumpSwap) && !migration) {
        ...
        const pre = new Set((tx.meta?.preTokenBalances || []).map((b) => b?.mint).filter(Boolean));
        const post = new Set((tx.meta?.postTokenBalances || []).map((b) => b?.mint).filter(Boolean));
...
async function honeypotRiskGate(mintPub) {
  ...
  if (freezeAuthority) return { ok: false, reason: `freeze_authority_present:${freezeAuthority}` };
  ...
  if (isToken2022) return { ok: false, reason: "token2022_blocked" };

  const isTokenProgram = ownerStr === TOKEN_PROGRAM_ID.toBase58();
  if (!isTokenProgram) return { ok: false, reason: `unknown_token_program_owner:${ownerStr || "null"}` };
...
export async function verifyTokenSecurity(mint) {
  ...
  log("rpc:", activeRpcUrl);
  log("PAGE_LIMIT:", PAGE_LIMIT, "HARD_CAP:", HARD_CAP, "RPC_TIMEOUT_MS:", process.env.RPC_TIMEOUT_MS || 15000);

  // 1) Pump.fun origin: API first
  log("\n[1] pumpfun api lookup...");
  const api = await pumpfunApiLookup(mintPub.toBase58());
  if (api.ok) {
    score += 20;
    reasons.push("pumpfun_origin_api");
    log("[1] api ok:", {
      complete: api.token?.complete,
      curve: api.token?.curve_address || null,
      solMarketCap: api.token?.sol_market_cap ?? null,
    });
  } else {
    log("[1] api failed:", api);

    // 1b) On-chain fallback
    log("\n[1b] scanning on-chain for pumpfun origin + migration...");
    try {
      const { isPumpFun, migration, lpMint, scanned } = await findPumpfunAndMigrationOnchain(mintPub);
      log("[1b] scan done:", { isPumpFun, migration, lpMint, scanned });
      ...
      if (!isPumpFun) {
        return {
          ...
          details: { isPumpFun, scanned, rpcUsed: activeRpcUrl, api },
        };
      }

      score += 20;
      reasons.push("pumpfun_origin_onchain");
    } catch (e) {
      // IMPORTANT: this is not "not pumpfun", it's "rpc failed"
      return {
        safe: false,
        score: 0,
        reasons: ["Pump.fun origin unknown (RPC error)"],
        details: { rpcUsed: activeRpcUrl, api, error: String(e?.message || e) },
      };
    }
  }

  // 2) Liquidity state (bonding curve OR migration)
  // If you want migration signature, you can still scan for it on-chain later.
  // For now we use the same liquidity gate as before.
  log("\n[2.5] checking liquidity proof...");
  // If you want: feed migration from scanner (you can store it above). For simplicity we keep null here.
  const liq = await checkLiquidityState(mintPub, null);
  log("[2.5] liquidity result:", liq);
  ...
  if (!liq.ok) {
    return {
      ...
      details: { liquidity: liq, rpcUsed: activeRpcUrl, api: api.ok ? api.token : api },
    };
  }
  ...
  // 3) Authority checks
  log("\n[3] checking mint/freeze authorities...");
  const { mintAuthority, freezeAuthority } = await getMintAuthorities(mintPub);
  log("[3] authorities:", { mintAuthority, freezeAuthority });
  ...
  if (mintAuthority) {
    return {
      ...
      details: { mintAuthority, freezeAuthority, rpcUsed: activeRpcUrl },
    };
  }
  score += 15;
  ...
  log("\n[4] running honeypot/sellability gate...");
  const hp = await honeypotRiskGate(mintPub);
  log("[4] honeypot gate:", hp);
  ...
  if (!hp.ok) {
    return {
      ...
      details: { honeypotGate: hp, rpcUsed: activeRpcUrl },
    };
  }
  score += 30;
  ...
  // 5) Distribution
  log("\n[5] checking holder concentration...");
  const dist = await top10Concentration(mintPub);
  log("[5] distribution:", dist);
  ...
  return {
    ...
    details: {
      ...
      pumpfunApi: api.ok ? api.token : api,
    },
  };
}