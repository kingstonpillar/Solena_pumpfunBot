...
async function rpcTryAll(opName, fn) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      lastErr = e;
      if (!isRetryableRpcError(e)) continue; // still try next, you preferred "keep trying"
      continue;
    }
  }
...
async function getMintParsedAndRaw(mintPub) {
  ...
  for (const url of pickRpcCandidates()) {
    ...
    try {
      const [raw, parsedResp] = await Promise.all([
        rpcOnCurrent("getAccountInfo(mint)", (c) => c.getAccountInfo(mintPub, COMMITMENT)),
        rpcOnCurrent("getParsedAccountInfo(mint)", (c) => c.getParsedAccountInfo(mintPub, COMMITMENT)),
      ]);

      if (raw) {
        return { raw, parsed: parsedResp?.value || null };
      }

      // raw null might be RPC lag, keep trying next RPC
      continue;
    } catch (e) {
      lastErr = e;
      continue;
  ...
  return { raw: null, parsed: null, err: lastErr ? String(lastErr?.message || lastErr) : null };
}

function extractMintAuthorities(parsedValue) {
  // parsedValue is ParsedAccountInfo (already .value), so no parsedValue.value here
...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  try {
    ...
    return {
      ...
      details: { input: mintOrRecord, mint: mintStr, resolver: resolverMeta, rpcUsed: activeRpcUrl },
    };
  }
