...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  const top10PctString = await Top10PCT(mintStr);  // Fetch dynamic top 10 percentage

  // Step 2: Strip the '%' sign and convert it to a number
  const numericPercentage = parseFloat(top10PctString.replace('%', '').trim());
  ...
  console.log(`Top 10 holders own: ${top10PctString} of the total supply.`);  // Keep the '%' in the output
