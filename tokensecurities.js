...
export async function verifyTokenSecurity(mint) {
  ...
  try {
    mintStr = normalizeMintInput(mint);
    mintPub = new PublicKey(mintStr);
  } catch {
    return {
      safe: false,
      score: 0,
      reasons: ["Invalid mint"],
      details: {},
    };
  }
  ...
  if (!raw?.value) {
    return {
      ...
      details: { rpcUsed: activeRpcUrl, mint: mintStr },
    };
  }
  ...
  if (!isTokenProgram && !isToken2022) {
    return {
      ...
      details: { owner: ownerStr, rpcUsed: activeRpcUrl, mint: mintStr },
    };
  }
  ...
  if (!isInitialized) {
    return {
      ...
      details: { rpcUsed: activeRpcUrl, decimals, mint: mintStr },
    };
  }
  ...
  if (mintAuthority) {
    return {
      ...
      details: { mintAuthority, freezeAuthority, rpcUsed: activeRpcUrl, mint: mintStr },
    };
  }
  score += 35;
  ...
  if (isToken2022) {
    ...
    if (r.dangerous) {
      return {
        ...
        details: { token2022: r, rpcUsed: activeRpcUrl, mint: mintStr },
      };
    }
