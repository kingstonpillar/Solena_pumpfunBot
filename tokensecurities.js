...
// ---------------- Helpers ----------------
// Function to extract mint authorities from parsed info
...
function token2022RiskFromParsed(parsedInfo) {
  // Extract extensions from parsed token info
  const exts = Array.isArray(parsedInfo?.extensions) ? parsedInfo.extensions : [];
  const extensions = exts.map(ext => String(ext).toLowerCase());

  // List of risky extensions
  const dangerous = extensions.some(extension => 
    extension.includes("transferhook") ||
    extension.includes("confidential") ||
    extension.includes("defaultaccountstate") ||
    extension.includes("nontransferable") ||
    extension.includes("transferfee") ||
    extension.includes("withheld") ||
    extension.includes("permanentdelegate") ||
    extension.includes("mintcloseauthority")
  );

  return { extensions, dangerous };
}
