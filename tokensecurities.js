...
async function getTop10Pct(c, mintPub, commitment = "confirmed") {
  ...
  if (!list.length) return { ok: false, pct: null, reason: "no_largest_accounts" };
...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  // ✅ details now knows mint + resolver
  ...
  // ✅ record mintErr early (even if we end up failing later)
  ...
  const TOP10_MAX_PCT_FRAC = Number(process.env.TOP10_MAX_PCT || 0.27);
  const TOP10_MAX_PCT_PCT = TOP10_MAX_PCT_FRAC * 100;

  let t10 = null;
  try {
    t10 = await rpcTryAll("top10", (c) => getTop10Pct(c, mintPub, COMMITMENT));
  } catch {
    t10 = null;
  }
  ...
  if (!t10?.ok || t10.pct == null) {
    ...
    details.distribution = { top10Pct: t10.pct, pct: t10.pct };

    if (t10.pct > TOP10_MAX_PCT_PCT) {
      // ✅ before returning, capture final rpcUsed after failovers
      details.rpcUsed = activeRpcUrl;

      return {
        safe: false,
        score: 0,
        reasons: [
          `top10_hard_fail_${t10.pct.toFixed(2)}_gt_${TOP10_MAX_PCT_PCT.toFixed(
            2
          )}`,
        ],
        details,
  ...
  // ✅ capture final rpcUsed after any failovers
  details.rpcUsed = activeRpcUrl;
