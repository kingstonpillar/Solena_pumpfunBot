...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  const t10 = await Top10PCT(mintPub);  // Correctly awaiting the Top10PCT function

  // Log the top 10 response for debugging
  console.log("Top 10 Response:", t10);
  ...
  if (t10?.pct == null) {
    ...
    // Now, if the top10Pct is over the threshold, we mark it accordingly as "safe"
    if (t10.pct > 0.27) { // MAX top 10 percent threshold
      score += 10;
      reasons.push(`top10_ok_${t10.pct.toFixed(2)}_gt_0.27`);
    } else {
      score += 20;
