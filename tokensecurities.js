...
dotenv.config();
import { getTop10FromHelius } from './heliusTop10.js';
...
export async function getTop10Pct(mintPub, commitment = "confirmed") {
  // Fetch token supply
  ...
  // Fetch top 10 holders using the function you exported from heliusTop10.js
  const top10Data = await getTop10FromHelius(mintPub.toBase58()); // Using the function to get top 10 data

  if (!top10Data || top10Data.length === 0) {
    return { ok: false, pct: null, reason: "no_top10_found" };
  }

  // Calculate percentage of top 10 holders
  let sumTop10 = 0n;
  ...
  // Calculate the percentage of top 10 holdings relative to total supply
