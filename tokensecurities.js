...
dotenv.config();
import { getTop10FromHelius } from './top10.js'; // Import the Top 10 logic
...
// ---------------- Timeout Handling ----------------
// A function to implement timeout for Solana RPC and Helius requests
...
function token2022RiskFromParsed(parsedInfo) {
  ...
  // Ensure that we are dealing with strings before calling toLowerCase
...
async function getTop10Pct(mintPub, commitment = "confirmed") {
  ...
  // Queue the Helius request with rate limiting and interval
  const top10Data = await queue.add(async () => {
    await delay(1000);  // Enforce a 1 second interval between requests

    // Helius request with timeout
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000); // 10 seconds timeout
