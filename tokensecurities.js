...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  // Initialize details with the `t10` field set to `null`
  const details = {
    input: mintOrRecord,
    ...
    distribution: null,
    mintErr: null,
    t10: null, // Add the t10 field to the details object
  };
  ...
  // Fetch Top 10 Data from Helius for both token types
 try {
    // Ensure we await the result of Top10PCT
    const t10 = await Top10PCT(mintPub); 
    details.t10 = t10;  // Store the result in the details object

    if (t10?.pct == null) {
      // If pct is not available, set as unknown
    ...
    // If the Top10PCT function fails (e.g., API call failure), mark as unknown
    reasons.push("top10_pct_unknown");
    details.distribution = { reason: "rpc_fail" };
}
  ...
  const out = {
    totalSupply: totalSupply, // BigInt value
    top10Sum: top10Sum, // BigInt value
    top10Pct: t10.pct,
    reasons: reasons,
    details: details
  };
  ...
  console.log(JSON.stringify(out, replacer, 2)); // Use the replacer
