...
export async function checkTokenSecurity(mintOrRecord) {
  ...
  // If top10Pct is a number (e.g., 17), then proceed
  if (typeof top10Pct === 'number') {
    // Step 2: Calculate the score based on the top 10 percentage
    const top10Score = calculateScore(top10Pct);  // Use the numeric percentage for score calculation

    // Step 3: Add the score from top 10 holders' percentage
    score += top10Score;
    ...
    // If top10Pct is not a number, handle accordingly
