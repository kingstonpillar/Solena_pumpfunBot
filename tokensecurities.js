...
// Free-RPC friendly change: RPC failures => UNKNOWN (safe=null), not unsafe.
...
function isRetryableRpcError(e) {
  ...
  return (
    // Added: free RPC providers often throw these
    msg.includes("402") ||
    msg.includes("payment required") ||
    msg.includes("out of cu") ||
...
// ---------------- FIXED SCANNER ----------------
// Key change stays: do NOT swallow RPC errors as "no signatures".
...
export async function verifyTokenSecurity(mint) {
  ...
  if (api.ok) {
    ...
    try {
      ...
      // Changed: RPC failure is UNKNOWN, not unsafe
      return {
        safe: null,
        score: null,
        reasons: ["Pump.fun origin unknown (RPC limited/unavailable)"],
