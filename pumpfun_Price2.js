// pumpfun_Price2.js (ESM, single file)
...
const JUP_API_KEY_SELL = "dd1a4008-858b-4a4a-8edd-3f1c61c20a01";
...
export async function getPumpFunPriceOnce(recordOrMint) {
  if (!recordOrMint) return { error: "missing_input" };

  const canon = await getCanonicalMint(recordOrMint, "confirmed");

  if (!canon?.ok || !canon.mint) {
    return {
      error: canon?.error || "unresolved_mint_identifier",
      mint: String(recordOrMint?.mint || recordOrMint || ""),
      resolver: canon?.resolver || null,
    };
  }

  const mint = String(canon.mint).trim();
  const resolverMeta = canon?.resolver || null;

  // decimals: prefer provided, else fetch on-chain
  let decimals = null;
  const maybeDecimals =
    typeof recordOrMint === "object" && recordOrMint ? Number(recordOrMint.decimals) : NaN;

  if (isValidDecimals(maybeDecimals)) {
  ...
  // 1) Jupiter Quote (do not crash if not tradable)
  if (isValidDecimals(decimals)) {
    try {
      const q = await getQuotePriceSOLPerToken(mint, decimals);
      if (q?.priceSOL && q.priceSOL > 0) {
        return { ...q, mint, decimals, resolver: resolverMeta };
      }
    } catch (e) {
      // Expected case: HTTP 400 not tradable
    }
  }
  ...
  try {
    const p = await getJupiterPriceSOLFallback(mint);
    if (p?.priceSOL && p.priceSOL > 0) {
      return { ...p, mint, decimals: isValidDecimals(decimals) ? decimals : null, resolver: resolverMeta };
    }
  } catch {}
  ...
  try {
    const d = await getDexScreenerPriceSOLFallback(mint);
    if (d?.priceSOL && d.priceSOL > 0) {
      return { ...d, mint, decimals: isValidDecimals(decimals) ? decimals : null, resolver: resolverMeta };
    }
  } catch {}
