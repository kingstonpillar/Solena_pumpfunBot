...
// Now self-contained: builds its own Connection with RPC failover (RPC_URL_1, RPC_URL_2),
...
async function tryResolveIfTokenAccount(pubkey, commitment) {
  // FIXED: correct signature for getParsedAccountInfo
...
// ---------------- Main resolver (no external conn needed) ----------------
// recordOrMint can be string OR object containing mint/pair/bondingCurve/curve fields
export async function resolveMintOnChain(recordOrMint, commitment = COMMITMENT) {
  const candidates = [];
  ...
  // 1) if candidate is 32 bytes:
  //    try mint account -> token account -> pump curve -> stop
  for (const s of uniq) {
    const pk = safePk(s);
    if (!pk) continue;
  ...
  // 2) if it is not 32 bytes, cannot query chain with it
  const first = uniq[0] || String(recordOrMint || "");
