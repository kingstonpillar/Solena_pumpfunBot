...
// - "xxxxx...pump" suffix (stripped ONLY if stripping yields a valid 32-byte pubkey)
...
// Strip ONLY if it ends with "pump" AND stripping yields a valid 32-byte pubkey.
// Otherwise, keep as-is because "pump" might be part of the real pubkey.
export function stripPumpSuffix(input) {
  const raw = String(input || "").trim();
  if (!raw) return "";

  if (!raw.toLowerCase().endsWith("pump")) return raw;

  const stripped = raw.slice(0, -4).trim();
  const len = b58Len(stripped);

  return len === 32 ? stripped : raw;
}

// Patch #2: do NOT call stripPumpSuffix here to avoid double-strip and confusion.
// This function should only answer: "does this string decode to 32 bytes?"
...
async function tryResolveIfTokenAccount(pubkey, commitment) {
  ...
  const normalized = stripPumpSuffix(mintStr);

  if (!is32BytePubkeyStr(normalized)) return null;
