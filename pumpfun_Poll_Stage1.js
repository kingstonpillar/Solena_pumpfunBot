...
// Start/Stop loop wrapper (NO overlap ticks)
...
const CURVE_TARGET_SOL = Number(process.env.CURVE_TARGET_SOL || 100);
const CURVE_PCT_THRESHOLD = Number(process.env.CURVE_PCT_THRESHOLD || 100);
...
function normalizeMintInput(input) {
  ...
  // 2) Try stripping trailing "pump" only if needed
...
// =====================================================
// START/STOP LOOP (NO overlap ticks) + state persisted
// =====================================================
...
export async function stopBondingDetector(reason = "manual") {
  ...
  console.log("[bondingDetector] stopped", { reason });
}

// Backward compatible aliases if you want
export const startStage1Bonding = startBondingDetector;
export const stopStage1Bonding = stopBondingDetector;

// Direct execution: node pumpfun_poll_stage1_bonding_50pct.js
