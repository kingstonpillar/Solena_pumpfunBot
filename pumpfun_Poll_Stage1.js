...
function logCandidatesAbove20Percent() {
  let candidatesAbove20 = [];

  // Check each candidate in the CURVE_WATCH map
  for (const [mint, record] of CURVE_WATCH.entries()) {
    if (record.lastCurvePct > 20) {
      candidatesAbove20.push({
        mint: record.mint,
        curvePct: record.lastCurvePct,
        solLiquidity: record.lastSolLiquidity,
      });
    }
  }

  // If we have any candidates with a curve percentage > 20%, log them
  if (candidatesAbove20.length > 0) {
    console.log(`Candidates with curvePct > 20%:`);
    candidatesAbove20.forEach((candidate) => {
      console.log(`Mint: ${candidate.mint}, CurvePct: ${candidate.curvePct}%, SOL Liquidity: ${candidate.solLiquidity}`);
    });
  } else {
    console.log("No candidates found with curvePct above 20%");
  }
}
...
function txTouchesProgram(tx, programId) {
  ...
  console.log("Checking if transaction touches program:", target);
...
function pickMints(meta) {
  ...
  for (const b of meta?.postTokenBalances || []) {
    ...
    console.log(`New mint detected: ${mint}`);  // Log new mints being added
    out.push(mint);
  }
...
const curve = bondingCurvePda(mintPublicKey);
...
async function processBlock(slot) {
  let block;
  try {
    console.log(`[processBlock] Fetching block for slot: ${slot}...`);
    ...
    console.log(`[processBlock] Block fetched for slot: ${slot}`);
  } catch (err) {
  ...
  console.log(`[processBlock] Processing ${txs.length} transactions in this block...`);
  ...
  for (const t of txs) {
    ...
    console.log(`[processBlock] Processing ${rawMintList.length} new mints`);
    ...
    console.log(`[processBlock] Found ${candidates.length} candidates in this transaction`);
    ...
    console.log(`[processBlock] Curve info fetched for ${candidates.length} candidates`);
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      if (curveInfo.curvePct >= CURVE_WATCH_MIN_PCT) {
        ...
        console.log(`[processBlock] Candidate ${mintStr} added to watchlist with curvePct: ${curveInfo.curvePct}%`);
      }
    }
  }

  // Call logCandidatesAbove20Percent after processing the block
  logCandidatesAbove20Percent();
}
async function refreshWatchedCurves() {
  ...
  // Log mints being processed for debugging
  console.log("Refreshing watched curves for mints:", mints);

  for (const rawMintStr of mints) {
    const mintStr = String(rawMintStr || "").trim();
    if (!mintStr) continue;

    // Only process mints that end with "pump"
    if (!mintStr.toLowerCase().endsWith("pump")) continue;

    try {
      // Validate mint as a PublicKey
      const mint = new PublicKey(mintStr);
      console.log(`Valid mint found: ${mintStr}`);
    } catch (error) {
      console.error(`Invalid mint address: ${mintStr}`);
      continue; // Skip invalid mints
    }

    candidates.push(mintStr);
  }

  // Fetch curve information for each valid mint, running the calls in parallel
  const results = await Promise.all(
    candidates.map((m) => getCurveInfoForMint(m))
  );

  // Log results for debugging
  console.log(`Fetched curve info for ${candidates.length} mints`);

  // Update the watchlist with fetched curve information
  for (let i = 0; i < candidates.length; i++) {
    const mintStr = candidates[i];
    const curveInfo = results[i];
    if (!curveInfo) continue;
...
// Function to initialize the detector state only once
async function initDetectorStateOnce() {
  const existing = safeReadArray(outPath); // Reads the existing state from the bonding_candidates file
  const pruned = pruneOldCandidates(existing, CANDIDATE_TTL_MIN); // Removes any expired candidates

  if (pruned.length !== existing.length) atomicWriteArray(outPath, pruned); // Write back the pruned state to the file

  seenMints = new Set(pruned.map((x) => x?.mint).filter(Boolean)); // Update seenMints based on the pruned data
  seenSigs = new Set(); // Reset seenSigs

  // Get the current Solana slot and set the starting point for scanning
  const currentSlot = await rpcLimited("getSlot(init)", (c) => c.getSlot(COMMITMENT));
  lastSlot = Math.max(0, currentSlot - STARTUP_BACKSCAN_SLOTS); // Start scanning a few slots back
...
async function runDetectorTick(label) {
  ...
  try {
    ...
    console.log(`[${label}] Tick started at ${started}`);

    // Ensure that the detector is not already running
    if (detectorTickRunning) {
      console.log("[bondingDetector] running tick at moment, previous tick still running");
      return;
    }

    detectorTickRunning = true;

    // Proceed only if stop was not requested
    if (detectorStopRequested) return;

    let slotNow;
    try {
      // Fetch the current slot from Solana RPC
      slotNow = await rpcLimited("getSlot(loop)", (c) => c.getSlot(COMMITMENT));
    } catch {
      return;
    }

    // Calculate the last slot to process
    const end = Math.min(slotNow, lastSlot + MAX_CATCHUP_SLOTS);

    // Process blocks from the last slot to the end slot
    for (let s = lastSlot + 1; s <= end; s++) {
      if (detectorStopRequested) break;
      await processBlock(s); // Process the block for each slot
      lastSlot = s; // Update the last processed slot
    }

    // Recheck all watched mints each tick
    await refreshWatchedCurves();

    // Prune outdated watchlist items
    pruneWatchlist();

    // Flush qualified candidates to the output
    await flushQualifiedCandidatesFromWatch();

    // Log the completion of the tick along with stats
    ...
    // Log any errors that occur during the tick
    console.error(`[${label}] Error during tick:`, err?.message || err);
  } finally {
    // Ensure the detectorTickRunning flag is reset, allowing future ticks
    detectorTickRunning = false;

    // Log the completion of the tick
    console.log(`[${label}] Tick completed in ${Date.now() - started} ms`);
  }
}
// ---------------- Start / Stop ----------------
