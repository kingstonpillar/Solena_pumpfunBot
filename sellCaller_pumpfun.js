...
async function trySellAndRemove(positions, pos, reason) {
  ...
  if (!res) {
    await sendTelegram(`⚠️ SELL failed\nMint: ${mint}\nReason: ${reason}\nError: no response`).catch(() => null);
  ...
  if (!isConfirmedSell) {
    ...
    await sendTelegram(
      `⚠️ SELL not confirmed\nMint: ${mint}\nReason: ${reason}\nStatus: ${res.reason || "not_confirmed"}`
    ).catch(() => null);
  ...
  await sendTelegram(`✅ SELL confirmed\nMint: ${mint}\nReason: ${reason}\nTx: ${res.signature}`).catch(() => null);
...
if (tSig?.action === "SELL") {
  ...
  if (out.removed) {
    ...
    // clear signal source for this mint
...
if (!Number.isFinite(price) || price <= 0) continue;
    ...
    // 4) TRAILING + LOCK (lock telegram once per new lock level per mint)

if (!pos.entryPrice) pos.entryPrice = entryPrice;

const trail = applyAdvancedGapTrailing(pos, price);

// lock telegram once per new lock level per mint
if (trail?.signal?.type === "LOCK_ON") {
  ...
  pos.trailing ??= {};
  pos.trailing.lock ??= { active: true, lockedProfitPct: locked, notifiedAt: null };

  const lockObj = pos.trailing.lock;

  const lastNotified = lockObj.notifiedAt;
  const shouldNotify = lastNotified == null || Number(lastNotified) !== locked;
...
// sell on any trailing or lock sell signal
if (trail?.shouldSell) {
