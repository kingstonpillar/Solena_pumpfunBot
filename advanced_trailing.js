...
function computeLockPct(profitPct, startAt = 400, step = 200) {
  ...
  const floor = Math.floor(profitPct / step) * step; // 425 -> 400
  const lock = floor - step; // 400 -> 200
...
t.lastProfitPct = profitPct;
  ...
  // If lock is already active, enforce it first (even if profit < 400 now)
  if (t.lock.active) {
    // Sell if profit drops to locked level
  ...
  // If profit is high enough, (re)compute lock level and activate/update it
...
t.nextStepTriggerPct = null;
    ...
    // notify once on activation OR when level increases
...
// 🔎 DEBUG LOG HERE
console.log(
  `[TRAIL] mint=${position.mint || "?"} profit=${profitPct.toFixed(2)}% ` +
