...
const TRAILING_FILE = "./trailing_state.json";
const TRAILING_STATE = loadMapFromFile(TRAILING_FILE);
...
function persistTrailingState(position, state) {
  ...
  // persist to disk
  saveMapToFile(TRAILING_STATE, TRAILING_FILE);
}
...
export function clearTrailingState(mintOrPosition) {
  ...
  const deleted = TRAILING_STATE.delete(mint);
