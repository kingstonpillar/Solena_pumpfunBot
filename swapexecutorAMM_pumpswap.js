...
export async function executeAmmMigrationBuy({
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    // ------------------ Calculate token out ------------------
    const solAmountLamports = BigInt(Math.floor((Number(amountRaw) || 0.01) * 1e9));
    const solAmountBN = new BN(solAmountLamports.toString());

    const globalConfig = ammSdk.decodeGlobalConfig((await onlineSdk.fetchGlobal()).data);
    const feeConfig = ammSdk.decodeFeeConfig((await onlineSdk.fetchFeeConfig()).data);
    ...
    // ------------------ AMM BUY ------------------
    const { instructions } = await ammSdk.buyBaseInput(pool, BigInt(tokenOut.toString()), slippageFrac, {
      global: globalConfig,
      feeConfig,
    });
