...
export async function executeAmmMigrationBuy({
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    // Ensure wallet ATA exists and normalize
    const { ata, ix: createAtaIx } = await getOrCreateATAIx(conn, wallet.publicKey, mintPk);
    const ataPk = ata instanceof PublicKey ? ata : new PublicKey(ata);
    ...
    // Get buy price in SOL
    const priceRes = await getPumpFunPriceOnce({ mint: mintPk }).catch(() => ({ priceSOL: null }));
    const buyPriceSOL = Number(priceRes?.priceSOL);
    if (!Number.isFinite(buyPriceSOL) || buyPriceSOL <= 0) throw new Error("Failed to get price");

    // Build swap instructions depending on token type
    let swapInstructions;
    const poolAccountInfo = await conn.getAccountInfo(poolPk, "confirmed");
    if (!poolAccountInfo) throw new Error("Pool account not found");

    try {
      const decodedPool = sdk.decodePool(poolAccountInfo.data);

      // Check required fields exist
      if (!decodedPool.coinCreator) console.warn("⚠️ coinCreator is undefined");
      if (!decodedPool.baseMint) console.warn("⚠️ baseMint is undefined");
      if (!decodedPool.quoteMint) console.warn("⚠️ quoteMint is undefined");

      const isToken2022 = decodedPool.isToken2022 || false;

      if (isToken2022) {
        swapInstructions = await sdk.buyToken2022Instructions({
          pool: poolPk,
          userTokenAccount: ataPk,
          amount: BigInt(amountRaw.toString()),
          slippage: slippageFrac,
        });
      } else {
        swapInstructions = await sdk.buyBaseInputInstructions({
          pool: poolPk,
          userTokenAccount: ataPk,
          amount: BigInt(amountRaw.toString()),
          slippage: slippageFrac,
      ...
      throw new Error(`Failed to decode pool or build swap instructions: ${err.message}`);
    }
