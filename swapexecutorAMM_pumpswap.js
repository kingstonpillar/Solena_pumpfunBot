...
export async function executeAmmMigrationBuy({ ammPoolPublicKey, mint, amountRaw, slippageFrac = 0.005 }) {
...
const swapRes = await ammSdk.buyInstructions(
  poolPk,
  ata,
  BigInt(tokenOut.toString()),
  slippageFrac
);

// Defensive check
if (!swapRes || !swapRes.instructions || !swapRes.instructions.length) {
  throw new Error("AMM buy returned invalid response or empty instructions");
}

const { instructions } = swapRes;
...
console.log(`✅ AMM Migration Buy confirmed: https://solscan.io/tx/${signature}`);
    ...
    const position = {
      ...
      tokenAccount: ata.toBase58(), // auto-created wallet ATA
    ...
    return position;
  }); // <-- closes withRpcFailover
}; // <-- closes executeAmmMigrationBuy