...
export async function executeAmmMigrationBuy({
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    // ------------------ FETCH & DECODE POOL ------------------
    const rawPoolAccount = await conn.getAccountInfo(poolPk, "confirmed");
    if (!rawPoolAccount?.data) throw new Error("❌ Pool account not found");
    const pool = ammSdk.decodePool(rawPoolAccount.data);
