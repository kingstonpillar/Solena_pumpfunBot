...
export async function executeAmmMigrationBuy({
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (connection) => {
    const ammSdk = new PumpAmmSdk({ connection });

    // ------------------ Ensure USER ATA ------------------
    const { ata, ix: createAtaIx } = await getOrCreateATAIx(connection, wallet.publicKey, mintPk);
    if (createAtaIx) {
      await sendV0Tx(
        [
    ...
    if (!Number.isFinite(buyPriceSOL) || buyPriceSOL <= 0)
      throw new Error("Failed to fetch a valid Pumpfun token price");

    // ------------------ Convert SOL to token amount ------------------
    ...
    // ------------------ Fetch Pool ------------------
    const pool = await ammSdk.getPool(poolPk);
    console.log("POOL OBJECT:", pool);

    if (!pool || !pool.pubkey) throw new Error(`❌ Pool not found or invalid: ${poolPk.toBase58()}`);

    // ------------------ AMM BUY ------------------
    const swapInstructions = await ammSdk.buyInstructions({
      pool,
    ...
    if (!swapInstructions?.instructions?.length)
