...
/**
 * Buy instructions for Token-2022 pool
 */
export async function buyToken2022Instructions({
  ...
  // Normalize all public keys to base58 strings
  const poolStr = toBase58(pool);
  const userAtaStr = toBase58(userTokenAccount);

  // Build instructions with normalized keys
  const instructions = await sdk.buyToken2022Instructions({
    pool: poolStr,
    userTokenAccount: userAtaStr,
    amount,
    slippage,
  });

  // Ensure all keys inside instructions are normalized
  instructions.forEach((ix) => {
    if (ix.keys) {
      ix.keys = ix.keys.map((k) => ({ ...k, pubkey: toBase58(k.pubkey) }));
    }
  });
...
/**
 * Buy instructions for SPL token / base input pool
 */
export async function buyBaseInputInstructions({
  ...
  const instructions = await sdk.buyBaseInputInstructions({
    pool: poolStr,
    userTokenAccount: userAtaStr,
    amount,
    slippage,
  ...
  // Normalize keys inside instructions
  instructions.forEach((ix) => {
    if (ix.keys) {
      ix.keys = ix.keys.map((k) => ({ ...k, pubkey: toBase58(k.pubkey) }));
    }
...
export async function executeAmmMigrationBuy({
  ...
  if (!ammPoolPublicKey || !mint || (!amountRaw && !process.env.SOL_TO_SPEND))
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    if (!Number.isFinite(buyPriceSOL) || buyPriceSOL <= 0) throw new Error("Failed to get buy price");

    // --------------- Fetch pool and detect type -----------------
    ...
    const isToken2022 = toBase58(poolAccount.owner) === toBase58(sdk.token2022ProgramId);

    const decodedPool = isToken2022
      ? sdk.decodePoolToken2022(poolAccount.data)
      : sdk.decodePool(poolAccount.data);

    if (!decodedPool) throw new Error("Failed to decode pool account");

    // --------------- Compute tokenOut based on SOL_TO_SPEND -----------------
    const solAmount = amountRaw ?? Number(process.env.SOL_TO_SPEND);
    if (!Number.isFinite(solAmount) || solAmount <= 0) throw new Error("Invalid SOL amount");

    const tokenOut = getBuyTokenAmountFromSolAmount({
      global: sdk.globalState,
      feeConfig: sdk.feeConfig,
      mintSupply: decodedPool.tokenTotalSupply,
      bondingCurve: decodedPool.bondingCurve,
      amount: solAmount,
    ...
    const swapInstructions = isToken2022
      ? await buyToken2022Instructions({
          ...
          amount: BigInt(tokenOut.toString()),
    ...
    if (!swapInstructions || !swapInstructions.length) throw new Error("AMM buy returned empty instructions");
