...
export async function executeAmmMigrationBuy({
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    // Detect Token-2022 vs SPL
    const isToken2022 = poolAccount.owner.toBase58() === sdk.token2022ProgramId.toBase58();
    ...
    const swapInstructions = isToken2022
      ? await sdk.buyToken2022Instructions({
          pool: poolPk,
          userTokenAccount: ata,
          amount: BigInt(amountRaw.toString()),
          slippage: slippageFrac,
        })
      : await sdk.buyBaseInputInstructions({
          pool: poolPk,
          userTokenAccount: ata,
          amount: BigInt(amountRaw.toString()),
          slippage: slippageFrac,
