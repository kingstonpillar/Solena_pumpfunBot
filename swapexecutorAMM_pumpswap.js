...
export async function executeAmmMigrationBuy({
  ammPoolPublicKey,
  mint,
  amountRaw,          // optional: token amount in raw units or SOL lamports
  slippageFrac = 0.005
}) {
  if (!ammPoolPublicKey || !mint)
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    // Use OnlinePumpSdk to fetch global and fee config
    const global = await onlineSdk.fetchGlobal();
    const feeConfig = await onlineSdk.fetchFeeConfig();

    const solAmountLamports = amountRaw ?? BigInt(Math.floor((Number(process.env.SOL_TO_SPEND) || 0.01) * 1e9));
    ...
    const swapInstructions = await ammSdk.buyBaseInput(
      poolPk,
      BigInt(tokenOut.toString()),
      slippageFrac
    );

    if (!swapInstructions?.instructions?.length) throw new Error("AMM buy returned empty instructions");
