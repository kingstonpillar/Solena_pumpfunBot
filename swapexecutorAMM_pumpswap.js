...
export async function executeAmmMigrationBuy({
  ...
  return withRpcFailover("executeAmmMigrationBuy", async (conn) => {
    ...
    if (!poolAccountInfo) throw new Error("Pool account not found on-chain");
    const pool = ammSdk.decodePool(poolAccountInfo.data);

    // Fetch and decode global & fee config
    const onlineSdk = new OnlinePumpSdk(conn);
    const globalConfigAccount = await onlineSdk.fetchGlobal();
    const feeConfigAccount = await onlineSdk.fetchFeeConfig();
    const global = ammSdk.decodeGlobalConfig(globalConfigAccount.data);
    const feeConfig = ammSdk.decodeFeeConfig(feeConfigAccount.data);

    // Fetch token price from Pumpfun price service
    const priceRes = await getPumpFunPriceOnce({ mint: mintPk }).catch(() => ({ priceSOL: null }));
    const buyPriceSOL = Number(priceRes?.priceSOL);
    if (!Number.isFinite(buyPriceSOL) || buyPriceSOL <= 0) throw new Error("Failed to get valid price");

    // Convert SOL amount to BN
    const solAmountLamports = BigInt(Math.floor((Number(process.env.SOL_TO_SPEND) || 0.01) * 1e9));
    const solAmountBN = new BN(solAmountLamports.toString());

    // Calculate token amount using bonding curve (placeholder for AMM)
    const tokenOut = getBuyTokenAmountFromSolAmount({
      global,
      feeConfig,
      mintSupply: solAmountLamports,
      bondingCurve: null,
      amount: solAmountBN,
      isNewBondingCurve: true,
    });

    console.log(`AMM Migration | SOL in: ${solAmountLamports} lamports | tokenOut(raw): ${tokenOut.toString()} | priceSOL: ${buyPriceSOL}`);

    // Build swap instructions
    const { instructions } = await ammSdk.buyBaseInput(pool, BigInt(tokenOut.toString()), slippageFrac, { global, feeConfig });
