...
// - Wallet decrypt
// - SOL + token ATA balance check inside main
...
// ---------------- CONFIG ----------------
const JUP_SWAP_BASE = process.env.JUP_QUOTE_BASE?.trim() || "https://api.jup.ag";
...
function pctToBps(slippagePct) {
  ...
  if (!Number.isFinite(pct) || pct <= 0) return 50; // 0.50%
...
async function fetchJupJson(url, body = null, timeoutMs = 12000) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`jup_http_${res.status}: ${text.slice(0, 500)}`);
    }
...
async function broadcastProofTx(wallet) {
  // 0-lamport transfer to self, proves bot is alive and can sign
  ...
  const { blockhash, lastValidBlockHeight } = await rpcLimited("getLatestBlockhash", (c) =>
  ...
  await rpcLimited("confirmTransaction(proof)", (c) =>
    c.confirmTransaction({ signature: sig, blockhash, lastValidBlockHeight }, COMMITMENT)
  );
