...
import { OnlinePumpSdk, PUMP_SDK, canonicalPumpPoolPda } from "@pump-fun/pump-sdk";
...
console.log("[CURVE_FETCH_SELLSTATE_ACCOUNTS]", {
  ...
  const sellIxs = await PUMP_SDK.sellInstructions({
    ...
    bondingCurve,
    associatedUserAccountInfo: storedAtaInfo,
    mint,
    user,
...
// ---------------- AMM SELL ----------------
async function buildAmmSell({ conn, ammPool, user, userTokenAccount, amountRaw, slippageFrac }) {
  const onlineSdk = new OnlinePumpAmmSdk(conn);
  const offlineSdk = new PumpAmmSdk(conn);

  const swapState = await onlineSdk.swapSolanaState(
    ammPool,
    user,
    userTokenAccount,
    undefined
...
export async function executeAutoSellPumpfun({
  ...
  return rpcLimited("autoSellPumpfun", async (conn) => {
    ...
    // If caller already has a pool, try AMM first
    if (poolPk) {
      try {
        ...
        const signature = await sendV0TxWithConn(
          conn,
          [
            ComputeBudgetProgram.setComputeUnitLimit({ units: 300_000 }),
            ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 50_000 }),
    ...
    try {
      ...
      const signature = await sendV0TxWithConn(conn, ixs, [wallet]);
    ...
    // Optional migration check only after curve fails
    if (!poolPk) {
      ...
      if (poolPk) {
        console.log(
          `✅ Dynamically resolved PumpSwap pool (${resolved.source}): ${poolPk.toBase58()}`
        );
    ...
    return {
      ok: false,
      retryable: true,
      mode: poolPk ? "amm_pending_or_failed" : "curve_failed",
      pool: poolPk ? poolPk.toBase58() : null,
      reason: curveFailureReason || "Curve sell failed and no valid AMM fallback succeeded",
    };
  });
}