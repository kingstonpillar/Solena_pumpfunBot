...
export async function executeAutoSellPumpfun({
  ...
  if (poolPk) {
    ...
    console.log(
      `ℹ️ No PumpSwap pool found yet for ${mintPk.toBase58()}, trying curve`
    );
  }
}

    // Try AMM first if we have a pool
    if (poolPk) {
      try {
    ...
    try {
      ...
      console.log(`ℹ️ Curve sell failed too: ${msg}`);

      return {
        ok: false,
        retryable: true,
        mode: poolPk ? "amm_pending_or_failed" : "unresolved",
