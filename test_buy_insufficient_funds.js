...
function stripPumpSuffix(mint) {
  const s = String(mint || "").trim();
  return s.toLowerCase().endsWith("pump") ? s.slice(0, -4).trim() : s;
}
...
async function main() {
  ...
  const stripped = stripPumpSuffix(mint);

  // validate base58 after stripping
  try {
    new PublicKey(stripped);
  } catch {
    console.error("❌ Mint is not a valid base58 public key after stripping pump:", stripped);
    process.exit(1);
  }
  ...
  console.log("strippedMintForValidation:", stripped);
  ...
  const res = await executePumpfunBuyFromBonding({
    candidate: { mint }, // pass raw, executor strips if needed
  });
